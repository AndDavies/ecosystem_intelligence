...
Research-agent output should land here before it is promoted into `app/supabase/seed` or a persisted Supabase environment.
...
Candidate batches live in `research/ingestion/candidate-batches/*.json`.

Source-lead batches live in `research/ingestion/source-leads/*.json`.
