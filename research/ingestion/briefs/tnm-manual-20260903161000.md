# Research Run Brief - tnm-manual-20260903161000

- Trigger: manual
- Mode: corpus_refresh
- Selected coverage view: supply
- Selected gap: organization-editorial-profile:null
- Reason: This production corpus segment selects 18 of 18 currently eligible published organizations for comprehensive, review-first editorial-profile refresh without automatic activation or publication.
- Review-packet capacity: 18 candidates; this is an operational envelope, not a discovery-yield or evidence threshold.
- Minimum prospects: 18
...
- Target candidates: 18
- Collection plan: research/ingestion/collection-plans-v1/tnm-manual-20260903161000.json
- Claim ledger: research/ingestion/claim-ledgers-v1/tnm-manual-20260903161000.json
...
10. Preview the deterministic guarded sequence with `pnpm research:finalize -- --run research/ingestion/runs/tnm-manual-20260903161000.json --plan`. The plan derives this non-writing smoke gate internally: `pnpm research:smoke -- --run research/ingestion/runs/tnm-manual-20260903161000.json --collection-plan research/ingestion/collection-plans-v1/tnm-manual-20260903161000.json --claims research/ingestion/claim-ledgers-v1/tnm-manual-20260903161000.json --prospects research/ingestion/prospect-inventories-v1/tnm-manual-20260903161000.json --signals research/ingestion/signal-batches-v1/tnm-manual-20260903161000.json --leads research/ingestion/source-leads-v2/tnm-manual-20260903161000.json --candidates research/ingestion/candidate-batches-v2/tnm-manual-20260903161000.json --check-only`.
11. When the complete batch is final and private intake is authorized, run `pnpm research:finalize -- --run research/ingestion/runs/tnm-manual-20260903161000.json --apply`. It validates, prepares any missing private organization-logo dispositions, validates again, smoke-checks without writing, generates review/staging artifacts only when candidates exist, imports only through the tracked path, and reconciles the exact run in production. Use `--file-only` instead when Admin Review intake is unavailable but local review/staging artifacts are explicitly needed.
...
- Published organizations: 598
- Published capabilities: 561
