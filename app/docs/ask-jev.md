...
The model is pinned to `jev-1.13.0` and rubric `tnm-relevance-v3`; owner-selected Codex and OpenAI answering models are not changed. Price assumption: US$0.042 per million input tokens, outputs free, checked September 18, 2026. Reconfirm pricing/limits before activation or model upgrades. No database migration, vector extension, new service or schedule is required.
...
One query-only Choice request identifies which of location, provider role, connectivity, remote operation and availability/maturity are actually requested. Beneficiary attributes are not provider requirements. Only applicable dimensions are checked against the best 32 organizations, using the complete reviewed winning-offering projection and its maturity/availability qualifications. These are catalogue selection hints, not source verification. No raw citation graph is loaded for the selection step; final-answer evidence is admitted separately. The outcomes are supported, contradicted, not established and not requested. This bounded dimension set is not exhaustive requirement verification. Raw Jev confidence is neither a publication gate nor an evidence rating.

Final ordering uses continuous relevance first, then fewer contradictions/more supported requested conditions only at an equal score, then lexical baseline rank. Score is an expected value; integer flooring is not a relevance band. Exact full organization/legal names and the latest valid IDs in referential follow-ups are pinned, within the total 16 slots. Other requested conditions and original qualifications remain for the answering model to assess. Unknown or low-confidence constraints do not cause rejection. Jev judgments are not included as factual evidence in its prompt.
...
Successful selection artifacts use an instance-local ten-minute cache keyed by question/conversation, the complete reviewed selection projection, mode and rubric. Expired entries refresh synchronously with per-instance in-flight coalescing; no paid background refresh occurs. Instance changes are cache misses, not a cross-instance reuse promise. Changed reviewed selection fields invalidate the key, and public-ID admission is repeated before answering. Failed/partial passes are never cached. Hit/coalesced diagnostics report zero **new** provider usage; scores are reused judgments, not another full inference pass. Fixed record ordering, payload fingerprints, pre-constraint top-32 scores, final supplied ranking/constraints and raw relevance cutoff margin help distinguish input changes from provider variation. Caching does not make new model inference deterministic.

The directory’s explicit “Find by meaning · AI” action first applies the existing region/type/taxonomy/map-bounds filters, then fuses genuinely separate lexical, weighted offering/application and role/taxonomy field ranks into at most 100 candidates. Jev scores that pool without OpenAI. It shows at most five organization links and optional explicit technology-filter buttons, labelled with candidate coverage. It never silently changes filters or calls Jev while typing. Late responses are discarded after query/filter changes. Public enabled traffic has service-backed minute/day limits; authenticated owner-pilot is uncapped. Public activation remains separately gated by evaluation and provider/privacy terms.
...
1. Run both **Lexical baseline** and **Full catalogue · fresh Jev** for each question. Use lexical first on Q1/Q3/Q5 and Jev first on Q2/Q4: ten manually initiated runs total. Keep the model fixed at the owner-selected Luna/low configuration.
2. Mark usefulness and record missing organizations or questionable claims. Compare useful supported results first, latency and cost second. Export once all ten are complete; failed attempts are retained.
3. Review that round before further calls. If useful, repeat fresh selection and **Same evidence · answer replay** for Q1/Q2/Q4 to separate selection variability from answer variability. The signed two-hour receipt rejects changed evidence before OpenAI.
4. Cache miss/hit/expiry and hybrid-50/100 are later bounded experiments, not mandatory clicks. Cache scope is explicitly one runtime instance. Full-catalogue scores remain the coverage reference; a smaller pool must retain human-relevant candidates.

Export the page’s JSON before leaving the tab. It contains successes and failures, run IDs/timestamps/deployment SHA, browser/server/provider/finalization timings, input/output/cached/cache-write tokens, Jev cost, coverage, pre/post selection IDs and fingerprints, private human ratings/notes, raw pre-finalizer answer and the actual supplied public passage/citation bindings. The latter evidence and raw answer are exported only by this owner-only route, never logged or placed in public telemetry. Compare medians and individual cold/warm cases; this small manual set does not establish p95 or the held-out recall gates. Grade useful top-five matches, missing relevant organizations, qualifications and source support. Report token counts for the existing answering model; do not estimate its dollar cost using TypeSafe’s rate. Any incomplete provider usage is a lower bound, including cancelled work. No paid evaluation was run during implementation.
...
## September 19 audit corrections

The owner authorized correction and release after the ten-run audit. Four semantic passes completed; Q2 paid for broad scoring then fell back at constraint packing. Continuous ranking replaces integer bands; query applicability is decided once; complete reviewed qualifiers remain in selection while raw source graphs move to final-answer assembly. Final evidence reads batch public admission across only selected offerings, limit citation fields to functional/operating qualifications, and exclude mission/need citations as supplier-function proof. Whole passages are retained; no source qualification is string-truncated. A shared bounded AbortSignal stops the Ask evidence reads. General public dossier hydration keeps its existing behavior.

The answer model and low reasoning effort remain unchanged. Functional fit no longer inherits source-confidence rank; stored evidence confidence is still displayed unchanged. Identical support points are deduplicated, must-have gaps still downgrade fit, and decisive gaps stay visible with adjacent suggestions. Discovery database errors throw before cache admission. Catalogue assembly rejects duplicate/mismatched admission sets. Failed test attempts now carry a safe stage/code and run ID; logs do not contain raw account, question, answer or source text.

The next owner round decides whether this is worth keeping. Quality improvement takes priority; no improvement is a reason to settle on the best measured production configuration and stop tuning, not to expand the experiment indefinitely. Public Jev activation remains gated. No schema migration or new service is needed.
