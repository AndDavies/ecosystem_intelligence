...
create table public.organization_company_profiles (
...
for each row execute function private.set_updated_at();
create trigger organization_company_profiles_set_updated_at before update on public.organization_company_profiles
for each row execute function private.set_updated_at();
...
alter table public.organization_company_profiles enable row level security;
...
create policy "published organization profiles are readable"
on public.organization_company_profiles for select to anon, authenticated
using (
  exists (
    select 1 from public.organizations organization_record
    where organization_record.id = organization_company_profiles.organization_id
...
for all to authenticated using ((select private.is_atlas_staff())) with check ((select private.is_atlas_staff()));
create policy "atlas staff manage organization profiles" on public.organization_company_profiles
for all to authenticated using ((select private.is_atlas_staff())) with check ((select private.is_atlas_staff()));
...
using (
  bucket_id = 'atlas-private-intake'
  and (owner_id = (select auth.uid()) or (select private.is_atlas_staff()))
);
