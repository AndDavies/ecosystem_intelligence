-- Add one nullable, reviewed executive synthesis to the organization dossier.
-- Existing records and historical candidates remain valid with a null value.
-- This migration is intentionally unapplied until the compatible application,
-- Admin Review preview and publication tests have passed.

alter table public.organizations
  add column executive_relevance_summary text
    check (
      executive_relevance_summary is null
      or length(trim(executive_relevance_summary)) between 80 and 1200
    );

comment on column public.organizations.executive_relevance_summary
is 'Optional human-reviewed decision snapshot synthesized only from already-supported public organization fields and reviewed connections; omitted when the evidence is insufficient.';
...
begin
  ...
  return case leaf
    ...
    when 'executiveRelevanceSummary' then 'executive_relevance_summary'
...
begin
  ...
  loop
    ...
    insert into public.organizations (
      ...
      current_activity_as_of, operating_context, canadian_footprint, executive_relevance_summary, reviewed_questions,
      ...
      organization_record->>'executiveRelevanceSummary',
    ...
    if organization_record->>'executiveRelevanceSummary' is not null
       and not private.has_public_field_citation('organization', new_organization_id, 'executive_relevance_summary') then
      raise exception 'Organization v3 candidate % lacks executive-relevance evidence.', candidate_row.id using errcode = '22023';
    end if;
...
begin
  ...
  loop
    ...
    loop
      ...
      if operation_record->>'operation' = 'set_field' then
        ...
        set name = case when operation_record->>'field' = 'name' then operation_record#>>'{after}' else name end,
            ...
            executive_relevance_summary = case when operation_record->>'field' = 'executive_relevance_summary' then operation_record#>>'{after}' else executive_relevance_summary end,
    ...
    if exists (
      ...
      if exists (select 1 from public.organizations where id = candidate_row.target_entity_id and executive_relevance_summary is not null)
         and not private.has_public_field_citation('organization', candidate_row.target_entity_id, 'executive_relevance_summary') then
        raise exception 'The executive profile has an uncited executive-relevance summary.' using errcode = '22023';
      end if;
