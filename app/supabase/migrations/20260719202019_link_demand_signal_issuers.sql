-- Require every manually maintained demand signal to retain a canonical issuing
-- authority. Replacing the primary issuer changes only the relationship row;
-- stable demand-source and requirement identifiers remain untouched.
...
begin
  ...
  insert into public.audit_events (
    ...
    case when is_new then 'Administrator created a published public demand signal.' else 'Administrator updated a published public demand signal without changing its dependent matches.' end,
    jsonb_build_object(
...
is 'Creates or edits a published demand signal transactionally, maintains its primary issuer, and preserves stable requirement identifiers and dependent matches.';
