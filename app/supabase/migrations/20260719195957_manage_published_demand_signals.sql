-- Maintain public demand sources and requirements through one transactional,
-- administrator-only operation. Existing requirement identifiers are preserved
-- so capability matches and citations remain connected when copy is edited.
...
begin
  ...
  if jsonb_typeof(p_payload) <> 'object'
    ...
    raise exception 'The demand signal payload is incomplete.' using errcode = '22023';
  end if;
  ...
  insert into public.audit_events (
    ...
    case when is_new then 'Administrator created a published public demand signal.' else 'Administrator updated a published public demand signal without changing its relationships.' end,
    jsonb_build_object('rationale', trim(p_rationale), 'source_id', canonical_source_id, 'relationship_ids_preserved', not is_new)
  );
...
is 'Creates or edits one published public demand source and its requirements transactionally while preserving existing requirement IDs and dependent relationships.';
