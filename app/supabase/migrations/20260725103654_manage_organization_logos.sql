-- Organization logos reuse the existing public-media bucket and media_assets
-- table. These invoker functions keep replacement/removal and audit logging in
-- one database transaction while retaining normal staff RLS enforcement.
...
create or replace function public.remove_published_organization_logo(
  p_organization_id uuid,
  p_reviewer_id uuid
)
returns text[]
...
begin
  ...
  insert into public.audit_events (
    ...
    'organization',
    p_organization_id,
    'Administrator removed the published organization logo.',
    jsonb_build_object('storage_paths', to_jsonb(v_storage_paths))
  );
...
revoke all on function public.remove_published_organization_logo(uuid, uuid) from public, anon;
...
grant execute on function public.remove_published_organization_logo(uuid, uuid) to authenticated;
