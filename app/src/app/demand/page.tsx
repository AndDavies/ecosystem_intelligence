    ...
    >
      {totalMatches === 0 ? (
        <div className="mb-5 flex items-start gap-3 rounded-xl border border-[var(--atlas-primary-border)] bg-[var(--atlas-primary-soft)] px-4 py-4 text-sm leading-6 text-[var(--atlas-primary)]">
          ...
          <div><p className="font-semibold">This overlay is intentionally marked as an early preview.</p><p className="mt-1 text-xs leading-5">The five public NATO problem families are loaded, but no organization matches have been assessed yet. Use the feedback control to tell us which demand-to-capability decisions would be most useful.</p></div>
        </div>
      ) : null}
      <div className="mb-5 flex items-start gap-3 rounded-lg border border-[var(--atlas-amber)] bg-[var(--atlas-amber-soft)] px-4 py-3 text-xs leading-5 text-[var(--atlas-amber)]">
        ...
        <p>NATO mappings are analyst assessments based on published sources. They are not procurement eligibility, endorsement, classified demand, or a formal opportunity.</p>
      </div>
        ...
        {snapshot.demandRequirements.map((demand) => (
          <PublicCard key={demand.id} className="flex h-full flex-col">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded bg-[var(--atlas-surface-muted)] px-2 py-1 text-[10px] font-semibold text-[var(--atlas-muted)]">{demand.matches.length} assessments</span>
            </div>
            <p className="mt-5 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--atlas-muted)]">Problem family {String(demand.displayOrder).padStart(2, "0")}</p>
            ...
            <div className="mt-auto pt-5">
              <Link href={`/demand/${demand.slug}`} className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--atlas-primary)] no-underline hover:underline">Open demand page <ArrowRight className="size-3.5" /></Link>
            </div>
          </PublicCard>
        ))}
      </div>
