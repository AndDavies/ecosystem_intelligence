...
export default async function ConnectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <PublicPageShell variant="form" eyebrow="Private connection request" title={`Connect with ${organization.name}`} description="Share the purpose of the conversation. Andrew reviews each request and may coordinate an introduction when it appears useful to both sides." backHref={`/organizations/${organization.slug}`} backLabel="Back to organization profile">
