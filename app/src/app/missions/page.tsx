      ...
      <section className="mt-9 sm:mt-11" aria-labelledby="mission-directory-heading">
        ...
        <div className="mt-7 grid gap-4 md:grid-cols-2">
          {snapshot.missions.map((item) => (
            <PublicCard key={item.missionArea.id} className="group relative flex h-full flex-col transition-colors" contentClassName="flex h-full flex-col">
              <div className="flex items-start justify-between gap-4">
                <TopicIcon title={item.missionArea.name} />
                <span className="rounded bg-[var(--atlas-surface-muted)] px-2 py-1 text-xs font-bold text-[var(--atlas-muted)] ring-1 ring-[var(--atlas-border)]">Our assessment</span>
              </div>
              <h3 className="mt-5 text-xl font-extrabold tracking-[-0.03em] text-[var(--atlas-ink)]">{item.missionArea.name}</h3>
              <p className="mt-3 flex-1 text-sm leading-6 text-[var(--atlas-muted)]">{item.missionArea.summary}</p>
              <dl className="mt-5 grid grid-cols-3 gap-3 border-t border-[var(--atlas-border)] pt-4 text-center">
                <div><dt className="text-xs text-[var(--atlas-muted)]">Published organizations</dt><dd className="mt-1 text-lg font-extrabold text-[var(--atlas-ink)]">{item.organizationCount}</dd></div>
                <div><dt className="text-xs text-[var(--atlas-muted)]">Published technologies</dt><dd className="mt-1 text-lg font-extrabold text-[var(--atlas-ink)]">{item.capabilityCount}</dd></div>
                <div><dt className="text-xs text-[var(--atlas-muted)]">Published Defence needs</dt><dd className="mt-1 text-lg font-extrabold text-[var(--atlas-ink)]">{item.connectedPublicNeedCount}</dd></div>
              </dl>
              <Link href={`/missions/${item.missionArea.slug}`} data-internal-link-role="contextual" data-internal-link-module="mission_collection" className="inline-flex min-h-11 items-center gap-1 pt-5 text-xs font-bold text-[var(--atlas-primary)] no-underline after:absolute after:inset-0 after:rounded-none after:content-[''] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--atlas-evidence)]">
...
function MissionStat({
  ...
  label: string;
  value: number;
  tone: keyof typeof missionStatIconTone;
}) {
  return (
    <div className="border-b border-[var(--atlas-border)] py-3">
      <div className="flex items-start gap-2 sm:items-center sm:gap-2.5">
        ...
        <div className="flex min-w-0 flex-col">
          <dt className="order-2 mt-1.5 text-xs font-bold leading-4 text-[var(--atlas-muted)] sm:text-[11px]">{label}</dt>
