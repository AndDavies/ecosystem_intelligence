...
export default async function LandingPage() {
  ...
  return (
    <main className="atlas-page tnm-landing min-h-screen bg-[var(--atlas-canvas)] text-[var(--atlas-ink)]">
      ...
      <section className="border-b border-[var(--atlas-border)] bg-white py-10 sm:py-14 lg:py-16" aria-labelledby="landing-heading">
        <div className="atlas-frame grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center lg:gap-16">
          <div>
            <p className="atlas-eyebrow">{brandCopy.category}</p>
            <h1 id="landing-heading" className="mt-4 max-w-3xl text-[clamp(2.4rem,4.3vw,4rem)] font-extrabold leading-[1.03] tracking-[-0.045em]">{brandCopy.headline}</h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-[var(--atlas-ink-soft)] sm:text-lg sm:leading-8">{brandCopy.introduction}</p>
          </div>
          <div className="min-w-0 rounded-[18px] bg-[var(--atlas-blue-soft)] p-5 sm:p-7">
            <h2 className="mb-4 text-xl font-extrabold">Who are you looking for?</h2>
            <PublicRecordSearch placement="home" />
            <p className="mt-3 text-sm text-[var(--atlas-muted)]">Try a company name, sonar or cybersecurity.</p>
            ...
            <p className="mt-3 text-sm font-semibold">{brandCopy.access}</p>
          </div>
        </div>
      </section>
...
function JobCard({ icon: Icon, title, text, label, href, entryPath }: { icon: typeof Search; title: string; text: string; label: string; href: string; entryPath: "directory" | "public_need" | "mission" }) {
  return <LandingEntryLink href={href} entryPath={entryPath} className="flex flex-col rounded-[18px] bg-white p-6 no-underline hover:bg-[var(--atlas-blue-soft)] hover:no-underline"><Icon className="size-6 text-[var(--atlas-evidence)]" aria-hidden="true" /><h3 className="mt-4 text-xl font-extrabold">{title}</h3><p className="mb-4 mt-3 text-sm leading-6 text-[var(--atlas-muted)]">{text}</p><span className="mt-auto inline-flex min-h-11 items-center gap-2 text-sm font-bold underline underline-offset-4">{label} <ArrowRight className="size-4" /></span></LandingEntryLink>;
}
