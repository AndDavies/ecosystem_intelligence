...
export function SignalEditorialDetails({ item }: { item: SignalItem }) {
  return <>
    {item.unknowns ? <div className="mt-6 max-w-[47rem] border-l-2 border-[var(--atlas-border-strong)] pl-5">
    ...
    <details className="group mt-6 rounded-xl border border-[var(--atlas-border)] px-5">
      ...
      <div className="space-y-5 border-t border-[var(--atlas-border)] py-5">
        <div>
          ...
          <SignalNarrative text={item.sourceFact} className="mt-2 max-w-[47rem] text-[15px] leading-7 text-[var(--atlas-ink-soft)]" />
        </div>
        {item.automatedRead ? <div>
          <h3 className="font-heading text-sm font-extrabold text-[var(--atlas-ink)]">True North Map assessment</h3>
          <SignalNarrative text={item.automatedRead} className="mt-2 max-w-[47rem] text-[15px] leading-7 text-[var(--atlas-ink-soft)]" />
        </div> : null}
      </div>
    </details>
  </>;
