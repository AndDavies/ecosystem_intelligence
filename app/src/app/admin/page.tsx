...
function AdminMetric({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) {
  return <div className="rounded-lg border border-[var(--admin-border)] bg-white p-5"><div className="flex items-center justify-between text-[var(--admin-action)]">{icon}<strong className="text-2xl text-[var(--admin-ink)]">{value}</strong></div><p className="mt-3 text-xs font-semibold text-[var(--admin-muted)]">{label}</p></div>;
}

function AdminLink({ title, detail, href }: { title: string; detail: string; href: string }) {
  return <Link href={href} className="rounded-lg border border-[var(--admin-border)] bg-white p-5 no-underline hover:border-[var(--admin-border-strong)] hover:no-underline"><h2 className="text-sm font-bold text-[var(--admin-ink)]">{title}</h2><p className="mt-2 text-xs leading-5 text-[var(--admin-muted)]">{detail}</p><span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-[var(--admin-action)]">Open <ArrowRight className="size-3.5" /></span></Link>;
}
