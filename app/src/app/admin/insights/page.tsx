...
export default async function AdminInsightsPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <PublicPageShell variant="admin" eyebrow="Private administration" title="Public-beta operations" description="Review participation, learn from discovery behaviour, and progress private workflows without turning the product into a CRM." actions={<span className="rounded bg-[#f2f4f7] px-3 py-2 text-xs font-semibold text-[#475467]">{user.role} · {user.email}</span>}>
      ...
      <section className="mt-7"><SectionHeading title="Recent searches" detail="Raw text expires after 90 days. Prioritize repeated coverage gaps, weak selections, and slow responses." /><div className="mt-4 space-y-2">{searches.data?.map((item) => { const assistant = assistantMeta(item.resolved_filters); return <div key={item.id} className={`rounded-md border p-3 text-xs ${item.zero_result ? "border-[#fedf89] bg-[#fffaeb]" : "border-[#d0d5dd] bg-white"}`}><div className="flex items-center justify-between gap-3"><strong>{item.query_text}</strong><span>{item.result_count} results</span></div><p className="mt-1 text-[#667085]">{assistant.mode === "assistant" ? `Ask True North · ${assistant.outcome ?? assistant.fallbackReason ?? "unknown outcome"}${assistant.latencyMs ? ` · ${(assistant.latencyMs / 1000).toFixed(1)}s` : ""}${assistant.inputTokens ? ` · ${assistant.inputTokens} input tokens` : ""}` : item.interpretation} · {item.context_path} · {new Date(item.created_at).toLocaleString("en-CA")}</p></div>; })}</div></section>
    </PublicPageShell>
  );
}
