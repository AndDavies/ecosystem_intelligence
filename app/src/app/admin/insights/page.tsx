...
export default async function AdminInsightsPage({ searchParams }: { searchParams: Promise<{ error?: string; preview?: string }> }) {
  ...
  const [connections, contacts, submissions, feedback, subscribers, searches, events, preferences, deliveryRuns, campaignMetrics] = await Promise.all([
...
async function readInsightEventPages(admin: ReturnType<typeof createAdminClient>, columns: string, since: string) {
  ...
  for (let from = 0; ; from += pageSize) {
    const result = await admin.from("pilot_events").select(columns).gte("created_at", since).order("created_at", { ascending: true }).range(from, from + pageSize - 1);
    if (result.error) return { data: [], error: result.error };
    const page = (result.data ?? []) as unknown as NewsletterInsightEvent[];
    data.push(...page);
