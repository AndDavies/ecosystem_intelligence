...
import { editPublishedOrganization } from "@/lib/actions/atlas-admin";
...
const errorMessages: Record<string, string> = {
  ...
  "update-failed": "The public record was not changed. Refresh the page and verify that its location, capability, and taxonomy values still exist."
};
...
export default async function EditPublishedOrganizationPage({
  ...
  const [{ data: domains }, { data: clusters }, { data: clusterLinks }] = await Promise.all([
    supabase.from("technical_domains").select("id, slug, name").eq("publication_status", "published").order("name"),
    supabase.from("ecosystem_clusters").select("id, slug, name").eq("publication_status", "published").order("name"),
  ...
  return (
    <PublicPageShell eyebrow="Published record maintenance" title={`Edit ${dossier.name}`} description="Changes save directly to the public atlas. Stable profile URLs are preserved and the complete before/after change is recorded in the audit log." backHref="/admin/organizations" backLabel="Published organizations" actions={<Link href={`/organizations/${dossier.slug}`} target="_blank" className="inline-flex h-10 items-center gap-2 rounded-md border border-[#d0d5dd] bg-white px-3 text-xs font-semibold text-[#344054] no-underline">View public profile <ExternalLink className="size-3.5" /></Link>}>
      ...
      {query.success ? <div className="mb-5 rounded-md border border-[#a6f4c5] bg-[#f6fef9] px-3 py-2 text-sm text-[#067647]">Published record updated and public atlas cache refreshed.</div> : null}

      {capabilities.length > 1 ? (
        <div className="mb-5 flex flex-wrap items-center gap-2 rounded-lg border border-[#d0d5dd] bg-white p-3">
          <span className="mr-1 text-xs font-semibold text-[#667085]">Capability being edited:</span>
      ...
      <form action={editPublishedOrganization} className="space-y-5">
        ...
        <PublicCard title={capability.name} eyebrow="Published capability">
          <div className="grid gap-4 md:grid-cols-2">
            <EditField label="Capability name"><input name="capabilityName" required maxLength={240} defaultValue={capability.name} className={fieldClass} /></EditField>
            <EditField label="Capability type"><input name="capabilityType" maxLength={240} defaultValue={capability.capability_type ?? ""} className={fieldClass} /></EditField>
          </div>
          <EditField label="Capability summary" className="mt-4"><textarea name="capabilitySummary" required minLength={40} maxLength={4000} rows={5} defaultValue={capability.summary} className={areaClass} /></EditField>
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            ...
            <EditField label="Defence applications"><textarea name="applications" rows={5} defaultValue={(capability.defence_applications ?? []).join("\n")} className={areaClass} /><FieldHint>One item per line.</FieldHint></EditField>
            <EditField label="Technical tags"><textarea name="tags" rows={5} defaultValue={(capability.technical_tags ?? []).join("\n")} className={areaClass} /><FieldHint>One item per line.</FieldHint></EditField>
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <EditField label="Primary technical domain"><select name="technicalDomainSlug" required defaultValue={primaryDomain?.slug ?? ""} className={fieldClass}><option value="" disabled>Select a domain</option>{domains?.map((domain) => <option key={domain.id} value={domain.slug}>{domain.name}</option>)}</select></EditField>
            <EditField label="Ecosystem cluster"><select name="clusterSlug" defaultValue={selectedCluster?.slug ?? ""} className={fieldClass}><option value="">No cluster</option>{clusters?.map((cluster) => <option key={cluster.id} value={cluster.slug}>{cluster.name}</option>)}</select></EditField>
            <EditField label="Capability confidence"><select name="capabilityConfidence" defaultValue={capability.source_confidence} className={fieldClass}><option value="high">High</option><option value="moderate">Moderate</option><option value="needs_review">Needs review</option></select></EditField>
          </div>
          <fieldset className="mt-4 rounded-md border border-[#d0d5dd] p-4">
            <legend className="px-1 text-xs font-semibold text-[#344054]">Additional technical domains</legend>
        ...
        <PublicCard title="Editorial decision" eyebrow="Immediate public change">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <EditField label="Organization confidence"><select name="organizationConfidence" defaultValue={dossier.source_confidence} className={fieldClass}><option value="high">High</option><option value="moderate">Moderate</option><option value="needs_review">Needs review</option></select></EditField>
            <EditField label="Freshness"><select name="freshnessStatus" defaultValue={dossier.freshness_status} className={fieldClass}><option value="current">Current</option><option value="review_due">Review due</option><option value="stale">Stale</option></select></EditField>
...
function EditField({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) {
  return <label className={`grid gap-1.5 text-xs font-semibold text-[#344054] ${className}`}>{label}{children}</label>;
}
