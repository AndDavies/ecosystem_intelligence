    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
      <PublicCard eyebrow="Edition" title="Page settings">
        <form action={updateSignalEdition} className="grid gap-4">
          ...
          {isV3 ? <>{[["opening", "Editorial opening", summary?.opening], ["takeaway", "Edition takeaway", summary?.takeaway], ["limitation", "Edition limitation (optional)", summary?.limitation]].map(([name, label, value]) => <SignalTextEditor key={name} name={name ?? ""} label={label ?? ""} defaultValue={value ?? ""} required={name !== "limitation"} rows={name === "opening" ? 12 : 5} />)}</> : <SignalTextEditor name="executiveSummary" label="Executive summary" defaultValue={String(edition.executive_summary)} rows={10} />}
