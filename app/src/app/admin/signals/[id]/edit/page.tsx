...
export default async function EditSignalEditionPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const [{ data: edition, error: editionError }, { data: itemData }, { data: socialData }] = await Promise.all([
    ...
    supabase.from("signal_items").select("*").eq("edition_id", id).order("position"),
  ...
  const [{ data: sourceLinks }, { data: recordLinks }] = itemIds.length ? await Promise.all([
    supabase.from("signal_item_sources").select("item_id, display_order, is_primary, signal_sources(*)").in("item_id", itemIds).order("display_order"),
    supabase.from("signal_record_links").select("*").in("item_id", itemIds).order("display_order")
  ]) : [{ data: [] }, { data: [] }];
  ...
  return <PublicPageShell variant="admin" eyebrow="Edit Signal edition" title={String(edition.title)} description="Correct the published editorial record, inspect its source lineage, and copy its private social examples. Saving refreshes the public Signals routes." breadcrumbs={[{ label: "Admin", href: "/admin" }, { label: "Signals", href: "/admin/signals" }, { label: "Edit edition" }]} actions={edition.publication_status === "published" ? <Link href={`/signals/${String(edition.slug)}`} target="_blank" className="atlas-secondary-button h-11 gap-2 px-4 text-sm">View public page <ExternalLink className="size-4" /></Link> : null}>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
      <PublicCard eyebrow="Edition" title="Page settings">
        <form action={updateSignalEdition} className="grid gap-4"><input type="hidden" name="editionId" value={id} /><label className="text-xs font-bold">Title<input name="title" defaultValue={String(edition.title)} className="mt-1 w-full rounded-xl border border-[var(--atlas-border)] bg-white px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">Executive summary<textarea name="executiveSummary" defaultValue={String(edition.executive_summary)} rows={10} className="mt-1 w-full rounded-xl border border-[var(--atlas-border)] bg-white px-3 py-2.5 text-sm leading-6" /></label><div className="flex flex-wrap items-center gap-3"><select name="publicationStatus" defaultValue={String(edition.publication_status)} className="h-11 rounded-xl border border-[var(--atlas-border)] bg-white px-3 text-sm"><option value="published">Published</option><option value="archived">Archived</option></select><button className="atlas-primary-button h-11 px-5 text-sm">Save edition</button></div></form>
      </PublicCard>
      <PublicCard eyebrow="Editorial visual" title="Published hero image">
        {heroUrl ? <><div className="relative aspect-video overflow-hidden rounded-2xl bg-[var(--atlas-surface-muted)]"><Image src={heroUrl} alt={String(edition.hero_image_alt ?? "Signal edition hero image")} fill sizes="(max-width: 1279px) 100vw, 36vw" className="object-cover" /></div><p className="mt-3 text-xs font-semibold text-[var(--atlas-ink-soft)]">{String(edition.hero_image_attribution ?? "Source attribution unavailable")}</p>{edition.hero_image_source_url ? <Link href={String(edition.hero_image_source_url)} target="_blank" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[var(--atlas-primary)]">Open image source <ExternalLink className="size-3.5" /></Link> : null}</> : <p className="text-sm text-[var(--atlas-muted)]">No hero image is stored for this edition.</p>}
      </PublicCard>
    </div>
    ...
    <section className="mt-6 space-y-5" aria-labelledby="edition-items-title"><div><p className="atlas-eyebrow">Article entries</p><h2 id="edition-items-title" className="mt-2 text-2xl font-extrabold tracking-[-0.035em]">Edit the published briefing</h2></div>{items.map((item) => {
      ...
      return <PublicCard key={itemId} eyebrow={`${String(item.position).padStart(2, "0")} · ${String(item.lane).replaceAll("_", " ")}`} title={String(item.title)}>
        <form action={updateSignalItem} className="grid gap-4"><input type="hidden" name="itemId" value={itemId} /><input type="hidden" name="editionId" value={id} /><label className="text-xs font-bold">Signal title<input name="title" defaultValue={String(item.title)} className="mt-1 w-full rounded-xl border border-[var(--atlas-border)] px-3 py-2.5 text-sm" /></label>{[["bottomLine", "Subheading", item.bottom_line], ["executiveSummary", "Executive summary", item.executive_summary], ["sourceFact", "What the public record says", item.source_fact], ["automatedRead", "Why this may matter", item.automated_read], ["unknowns", "What remains unknown", item.unknowns], ["nextStep", "Practical next step", item.next_step]].map(([name, label, value]) => <label key={String(name)} className="text-xs font-bold">{String(label)}<textarea name={String(name)} defaultValue={String(value)} rows={name === "executiveSummary" ? 9 : 4} className="mt-1 w-full rounded-xl border border-[var(--atlas-border)] px-3 py-2.5 text-sm leading-6" /></label>)}<fieldset><legend className="text-xs font-bold">Searchable tags</legend><div className="mt-2 flex flex-wrap gap-2">{signalTagDefinitions.map((tag) => <label key={tag.id} className="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-full border border-[var(--atlas-border)] bg-white px-3 py-2 text-xs font-semibold"><input type="checkbox" name="tags" value={tag.id} defaultChecked={selectedTags.includes(tag.id)} className="accent-[var(--atlas-primary)]" />{tag.label}</label>)}</div></fieldset><div className="flex flex-wrap items-center gap-3"><select name="confidence" defaultValue={String(item.confidence)} className="h-11 rounded-xl border border-[var(--atlas-border)] bg-white px-3 text-sm"><option value="high">High</option><option value="medium">Medium</option><option value="limited">Limited</option></select><button className="atlas-primary-button h-11 px-5 text-sm">Save article entry</button></div></form>
        <div className="mt-6 grid gap-4 lg:grid-cols-2"><div className="rounded-2xl bg-[var(--atlas-surface-muted)] p-4"><h3 className="text-sm font-extrabold">Original sources</h3><div className="mt-3 space-y-3">{sources.map((link, index) => { const source = oneRelation(link.signal_sources); return source ? <Link key={`${itemId}-source-${index}`} href={String(source.canonical_url)} target="_blank" className="block rounded-xl bg-white p-3 text-xs no-underline hover:text-[var(--atlas-primary)] hover:no-underline"><span className="font-extrabold">{String(source.title)}</span><span className="mt-1 block text-[var(--atlas-muted)]">{String(source.publisher)} · {String(source.evidence_locator)}</span></Link> : null; })}{!sources.length ? <p className="text-xs text-[var(--atlas-muted)]">No linked source was returned.</p> : null}</div></div><div className="rounded-2xl bg-[var(--atlas-blue-soft)] p-4"><h3 className="text-sm font-extrabold">Atlas continuations</h3><div className="mt-3 space-y-2">{links.map((link) => <Link key={String(link.id)} href={String(link.public_href)} target="_blank" className="block text-xs font-bold text-[var(--atlas-primary)]">{String(link.relationship_label)} <ExternalLink className="ml-1 inline size-3" /></Link>)}{!links.length ? <p className="text-xs text-[var(--atlas-muted)]">No atlas relationships were linked to this entry.</p> : null}</div></div></div>
      </PublicCard>;
    })}</section>

    <section className="mt-6" aria-labelledby="social-examples-title"><div><p className="atlas-eyebrow">Private distribution copy</p><h2 id="social-examples-title" className="mt-2 text-2xl font-extrabold tracking-[-0.035em]">Social examples</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--atlas-muted)]">These are generated with the edition and remain view-and-copy only. Copying does not post externally or change their stored status.</p></div><div className="mt-4 grid gap-4 lg:grid-cols-2">{socialExamples.map((draft) => <SignalSocialExample key={String(draft.id)} platform={String(draft.platform)} status={String(draft.status)} text={String(draft.draft_text)} />)}{!socialExamples.length ? <div className="rounded-2xl bg-[var(--admin-warning-soft)] p-5 text-sm"><p className="font-extrabold">No social examples are associated with this edition.</p><p className="mt-1 text-xs leading-5">Older editions may predate the current LinkedIn and X publication contract.</p></div> : null}</div></section>
  </PublicPageShell>;
}
