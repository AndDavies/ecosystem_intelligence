...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages = ["/", "/organizations", "/missions", "/regions", "/demand", "/briefs", "/about", "/how-it-works", "/methodology", "/contact", "/privacy", "/terms"];
  const regionSlugs = ["canada", "atlantic-canada", "quebec", "ontario", "prairies", "british-columbia", "north"];
  return [
    ...staticPages.map((path) => ({ url: absoluteUrl(path), lastModified: releaseUpdatedAt, changeFrequency: path === "/" ? "daily" as const : "monthly" as const, priority: path === "/" ? 1 : 0.6 })),
