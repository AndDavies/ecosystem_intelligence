...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...regionSlugs.map((slug) => ({ url: absoluteUrl(`/regions/${slug}`), changeFrequency: "weekly" as const, priority: 0.7 })),
    ...slugs.demands.map((record) => ({ url: absoluteUrl(`/demand/${record.slug}`), ...(record.updatedAt ? { lastModified: new Date(record.updatedAt) } : {}), changeFrequency: "monthly" as const, priority: 0.65 })),
