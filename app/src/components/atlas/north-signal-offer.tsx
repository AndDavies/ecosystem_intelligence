...
export function NorthSignalThisWeekCard({
  ...
  if (loading) {
    return (
      <div
        ...
        className={cn("rounded-[14px] bg-[var(--atlas-blue-soft)] px-4 py-4 text-[var(--atlas-ink)] sm:px-5", className)}
      >
  ...
  if (!proof) {
    return (
      <Link
        ...
        className={cn(
          "inline-flex min-h-11 items-center text-sm font-extrabold text-[var(--atlas-primary)] underline decoration-2 underline-offset-4",
          className
        )}
      >
  ...
  return (
    <Link
      ...
      className={cn(
        "group block rounded-[14px] bg-[var(--atlas-blue-soft)] px-4 py-4 text-[var(--atlas-ink)] no-underline transition-colors hover:bg-[var(--atlas-blue-soft-hover)] hover:no-underline focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--atlas-signal-soft)] sm:px-5",
        className
      )}
    >
      <span aria-hidden="true" className="mb-3 block h-1 w-10 rounded-full bg-[var(--atlas-signal)]" />
...
export function NorthSignalValueLines({ className }: { className?: string }) {
  return (
    <div data-north-signal-value-lines className={cn("grid gap-1.5 text-sm leading-6 text-[var(--atlas-ink-soft)]", className)}>
      {northSignalOffer.valueLines.map((line) => <p key={line}>{line}</p>)}
    </div>
  );
}
