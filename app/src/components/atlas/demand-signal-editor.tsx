...
import { upsertPublishedDemandSignal } from "@/lib/actions/atlas-admin";
...
const fieldClass = "rounded-md border border-[#d0d5dd] bg-white px-3 py-2 text-sm text-[#101828] outline-none focus:border-[#0756d9]";
    ...
    <form action={upsertPublishedDemandSignal} className="grid gap-5">
      ...
      <div className="space-y-4">
        {signal.requirements.map((requirement, index) => (
          <section key={requirement.id || index} className="rounded-lg border border-[#d0d5dd] bg-[#f8fafc] p-4">
            <h3 className="text-sm font-bold text-[#101828]">Public problem statement {index + 1}</h3>
            {requirement.id ? <p className="mt-1 text-[11px] text-[#667085]">Stable record: {requirement.slug}. Editing preserves every linked technology match.</p> : null}
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <PendingButton type="submit" pendingLabel="Saving…" className="h-11 bg-[#0756d9] px-5 text-sm font-semibold text-white"><Save className="mr-2 size-4" />{signal.id ? "Save demand signal" : "Publish new demand signal"}</PendingButton>
      </div>
    </form>
  );
}
...
function Field({ label, help, children }: { label: string; help?: string; children: React.ReactNode }) {
  return <label className="grid gap-1.5 text-xs font-semibold text-[#344054]"><span>{label}</span>{children}{help ? <span className="text-[11px] font-normal leading-5 text-[#667085]">{help}</span> : null}</label>;
}
