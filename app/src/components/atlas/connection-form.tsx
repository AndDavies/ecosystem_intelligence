...
export function ConnectionForm({ organizationId, organizationName, defaultName }: { organizationId: string; organizationName: string; defaultName: string }) {
  ...
  if (state === "success") return <div className="rounded-lg border border-[#9ee2c0] bg-[#edfcf4] p-5 text-sm leading-6 text-[#05603a]"><CheckCircle2 className="mb-2 size-5" /><strong>Request received.</strong> Andrew will privately review the fit and, where appropriate, help coordinate a consent-based introduction. No introduction or endorsement is automatic.</div>;
  ...
  return (
    <form onSubmit={submit} className="grid gap-4">
      ...
      <p className="text-[11px] leading-5 text-[#667085]">Ecosystem Intelligence facilitates discovery and potential introductions. It does not endorse either party, share private contact details publicly, or replace due diligence. See the <Link href="/terms" className="underline">Terms</Link>.</p>
      <button type="submit" disabled={state === "loading"} className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[#007f98] px-5 text-sm font-semibold text-white hover:bg-[#00677d] disabled:opacity-60 sm:w-fit">{state === "loading" ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />}Request connection</button>
    </form>
  );
}
