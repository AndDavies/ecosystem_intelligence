...
export function LandingEditorialPaths() {
  const { data, failed } = useLandingData();
  if (!data) return <SectionFallback label={failed ? "Mission Areas and Canadian Defence Signals are temporarily unavailable." : "Loading Mission Areas and Canadian Defence Signals…"} />;
  return <>
    ...
    <section className="border-t border-[var(--atlas-border)] bg-white py-16 sm:py-20" aria-labelledby="signals-heading"><div className="atlas-frame grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-14"><div><p className="atlas-eyebrow">Canadian Defence Signals</p><h2 id="signals-heading" className="mt-4 text-4xl font-extrabold tracking-[-0.045em] sm:text-5xl">See what changed, then follow the evidence.</h2><p className="mt-5 max-w-md text-base leading-7 text-[var(--atlas-muted)]">Signals identify consequential developments, distinguish public facts from assessment and connect the record to Canadian organizations, Mission Areas and Public Needs.</p><LandingEntryLink href="/signals" entryPath="north_signal" className="mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-[var(--atlas-evidence)] underline decoration-[var(--atlas-evidence)] decoration-2 underline-offset-4">Read recent Signals <ArrowRight className="size-4" /></LandingEntryLink></div><div className="grid gap-6 md:grid-cols-2">{data.signals.map((signal) => <LandingEntryLink key={signal.slug} href={`/signals/${signal.slug}`} entryPath="north_signal" aria-label={`Read Canadian Defence Signal: ${signal.title}`} className="group overflow-hidden rounded-[14px] border border-[var(--atlas-border)] bg-white text-[var(--atlas-ink)] no-underline transition-shadow hover:shadow-[var(--atlas-shadow-soft)] hover:no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--atlas-evidence)] focus-visible:ring-offset-4">{signal.heroImage ? <div className="relative h-[220px] overflow-hidden bg-[var(--atlas-ink)] sm:h-[250px]"><Image src={signal.heroImage.url} alt={signal.heroImage.alt} fill sizes="(max-width: 767px) 100vw, 35vw" className="object-cover" /></div> : <div className="h-[220px] bg-[var(--atlas-ink)] sm:h-[250px]" />}<div className="p-6"><p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--atlas-evidence)]">Signals · {new Intl.DateTimeFormat("en-CA", { dateStyle: "medium" }).format(new Date(`${signal.editionDate}T12:00:00Z`))}</p><h3 className="mt-4 text-2xl font-extrabold leading-tight tracking-[-0.035em]">{signal.title}</h3><p className="mt-4 line-clamp-3 text-base leading-7 text-[var(--atlas-muted)]">{signal.summary}</p><span className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm font-bold underline decoration-[var(--atlas-signal)] decoration-2 underline-offset-4">Read the edition <ArrowRight className="size-4" /></span></div></LandingEntryLink>)}</div></div></section>
  </>;
}
