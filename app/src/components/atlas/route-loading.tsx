  ...
  return (
    <>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.9fr)]">
        <div className="h-[430px] rounded-2xl border border-[var(--atlas-border)] bg-white" />
        <div className="space-y-3 rounded-2xl border border-[var(--atlas-border)] bg-white p-4">
    ...
    <>
      ...
      <div className={`mt-8 grid gap-4 ${review ? "grid-cols-1" : "md:grid-cols-2 xl:grid-cols-3"}`}>
        {Array.from({ length: review ? 3 : 6 }, (_, index) => (
          <div key={index} className={`${review ? "h-52" : "h-64"} rounded-2xl border border-[var(--atlas-border)] bg-white p-5`}>
