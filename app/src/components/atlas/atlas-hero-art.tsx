...
export function AtlasHeroArt({
    ...
    <>
      {imageSrc ? (
        <>
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/5 to-black/10" aria-hidden="true" />
        </>
      ) : (
        <>
  ...
  const classNames = `pointer-events-none relative isolate w-full min-w-0 overflow-hidden bg-gradient-to-br ${toneClass[tone]} ${compact ? "aspect-[16/9]" : "aspect-[16/7] min-h-[220px]"} ${className}`;
