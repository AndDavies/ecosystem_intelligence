...
const recordSchema = z.object({
  ...
  capability: z.object({
    ...
    technicalDomainSlug: z.string().min(1), additionalTechnicalDomainSlugs: z.array(z.string().min(1)).default([]),
    clusterSlug: z.string().nullable().optional(), alignmentSummary: z.string().min(40).optional(),
    missionMatches: z.array(z.object({
      missionAreaSlug: z.string().min(1), alignmentSummary: z.string().min(40), confidence: z.enum(["high", "moderate"])
    })).default([])
  }),
...
const batchSchema = z.discriminatedUnion("status", [
  z.object({
    batchId: z.string().min(1), title: z.string().min(1), status: z.literal("candidate"),
    createdAt: z.string().datetime(), scope: z.string().min(1), missionAreaSlug: z.string().min(1).optional(),
  ...
  z.object({
    ...
    scope: z.string().min(1).optional(), missionAreaSlug: z.string().min(1).optional(), records: z.array(recordSchema).min(1)
  })
]);
