...
if (!process.argv.includes("--live") || !process.argv.includes("--allow-paid") || !process.argv.includes("--allow-production-read")) {
  throw new Error("Live answer evaluation requires --live --allow-paid --allow-production-read; use assistant:eval for offline selection checks.");
}
loadScriptEnv();
...
async function main() {
  ...
  for (const testCase of cases) {
    ...
    if (run.metrics.latencyMs > 25_500) failures.push(`${testCase.name}: exceeded the 20 second answer plus 5 second selection budget`);
