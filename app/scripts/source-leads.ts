...
export const sourceLeadDir = path.join(workspaceRoot, "research", "ingestion", "source-leads");
...
export function formatSourceLeadValidationReport(results: SourceLeadValidationResult[]) {
  ...
  if (!results.length) {
    lines.push("");
    lines.push("No source lead batches found. Add JSON files to research/ingestion/source-leads when research begins.");
  }
