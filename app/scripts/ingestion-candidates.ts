...
export const researchDir = path.join(workspaceRoot, "research");
export const candidateBatchDir = path.join(researchDir, "ingestion", "candidate-batches");
export const candidateReviewDir = path.join(researchDir, "ingestion", "reviews");
export const candidatePromotionDir = path.join(researchDir, "ingestion", "promotions");
...
export function formatCandidateValidationReport(results: CandidateValidationResult[]) {
  ...
  results.forEach((result) => {
    ...
    lines.push(`File: ${path.relative(workspaceRoot, result.filePath)}`);
    lines.push(
    ...
    lines.push(`Promoted: ${result.promoted ? path.relative(workspaceRoot, result.promotionPath) : "no"}`);
...
export function formatCandidateReviewPacket(batch: CandidateBatch, result: CandidateValidationResult) {
  ...
  lines.push(`- Candidate file: \`${path.relative(workspaceRoot, result.filePath)}\``);
  ...
  lines.push("");
  lines.push("```bash");
  lines.push(`pnpm ingest:promote ${path.relative(workspaceRoot, result.filePath)} --reviewer "Reviewer Name"`);
  lines.push("```");
  lines.push("");
  lines.push("Preview the promotion without writing seed files:");
  lines.push("");
  lines.push("```bash");
  lines.push(`pnpm ingest:promote ${path.relative(workspaceRoot, result.filePath)} --reviewer "Reviewer Name" --dry-run`);
  lines.push("```");
...
export async function promoteCandidateBatch(input: {
  ...
  if (!dryRun && (await fileExists(promotionPath))) {
    throw new Error(
        `Batch ${input.batch.batchId} already has a promotion log at ${path.relative(workspaceRoot, promotionPath)}.`
    );
  }
  ...
  const promotionLog = {
    ...
    candidateFile: path.relative(workspaceRoot, input.filePath),
