...
describe("daily Signals contract", () => {
  ...
  it("keeps RLS, stable slugs, admin correction, public routes and private social drafts explicit", async () => {
    ...
    expect(detail).toContain("<SignalVisual visual={leadVisual} priority />");
    expect(detail).toContain("atlas-signal-masthead");
    expect(detail).toContain("edition.isLocalPreview ? {} : await getAtlasOrganizationLogos");
    expect(detail).toContain("atlas-signal-summary");
    expect(detail).toContain("atlas-signal-reading-grid");
    ...
    expect(articleNavigation).toContain("border-l-2");
    expect(articleNavigation).toContain("font-bold !text-[var(--atlas-ink)]");
    ...
    expect(archiveBrowser).toContain("atlas-signal-teaser");
    expect(archiveBrowser).toContain("signalLeadVisual(edition)");
    expect(archiveBrowser).toContain("atlas-signal-button");
    ...
    expect(northSignal).toContain('atlas-newsletter-section');
    expect(northSignal).toContain('tone === "dark"');
