...
describe("daily Signals contract", () => {
  ...
  it("keeps RLS, stable slugs, admin correction, public routes and private social drafts explicit", async () => {
    const [migration, rlsFix, archive, archiveBrowser, detail, articleNavigation, articleShare, tagPill, heroComponent, northSignal, taxonomy, admin, publisher, header, sitemap] = await Promise.all([
