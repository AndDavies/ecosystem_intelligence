...
describe("organization-type discovery", () => {
  it("constrains company searches by both organization type and region", async () => {
    const discovery = await discoverAtlas("companies in Ontario");
    ...
    const result = await queryAtlas({ region: "ontario", type: "company", pageSize: 100 });
  ...
  it.each([
    ...
    const discovery = await discoverAtlas(query);
  ...
  it("recognizes the remaining public organization types", async () => {
    await expect(discoverAtlas("accelerators in Canada")).resolves.toMatchObject({ filters: { type: "accelerator" } });
    await expect(discoverAtlas("research centres in Canada")).resolves.toMatchObject({ filters: { type: "research_test_centre" } });
    await expect(discoverAtlas("ecosystem organizations in Canada")).resolves.toMatchObject({ filters: { type: "ecosystem_organization" } });
  });
});
