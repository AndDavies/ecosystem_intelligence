Historical snapshot; not an active operating contract. Current authority: [current document](../../governance/Project%20Status.md).
...
Andrew has chosen manual morning invocation from this chat. The Daily Signals automation and scheduling contract were deleted on September 5; there is no paused schedule to reactivate. The v3 application and migration are deployed, and the revised private skill is installed after runtime verification.
...
The main-checkout Node 24 release gate passed 708 tests, lint, scale validation and the production build. All eleven installed skills validate, the revised helper passes 17 tests, and the operator validator confirms only the two unrelated schedules remain. The additive migration, v3 deployment, historical routes, stable RSS and responsive/keyboard reading checks pass. Daily Signals automation deletion is verified in the app and local scheduler store. The live `/api/system/signals-contract` and deployment SHA identify operational v3 support. Today's edition is deliberately deferred to a separately requested run; an existing Atlantic date receives a private comparison unless correction is explicitly authorized.

### September 4 governed canonical-organization repair

A production-wide read-only canonical review assessed 21 published organization records with identity, lifecycle, successor, entity-kind, alias, domain or invalid-child concerns. Seventeen have defensible evidence-backed repair treatments. Coastal Defence Systems, Industrial Recon, SkyX and AerialX remain explicit `research_required` holds rather than being forced into the queue. The companion audit covered all 598 published organizations, 561 capabilities and 458 aliases for normalized identity collisions, entity-kind contradictions, websites/redirects and historical canonical-stop records.
...
- Canadian Defence Signals is a manually invoked, publication-driven editorial surface. V3 provides substantial significance-led editions, immutable item evidence, optional source images, retryable private social examples and atomic publication. Exact historical v1/v2 repair remains supported. The skill writes only isolated `signal_*` tables and `brief-images/signals/`; nonpublishing outcomes create no edition or alert. No Daily Signals automation or scheduling contract remains.
...
4. Invoke the revised Daily Signals skill manually from this chat when an edition is wanted. Check the Atlantic date and existing edition first, then assess substantive intelligence, source integrity and reader value. An existing date receives a private comparison without silent replacement. No scheduled run is configured. Defence Brief and weekly North Signal retain their separate editorial workflows.
...
The existing weekly visibility automation is active for Monday 08:00 Halifax with bounded five-route checks, all configured providers, typed import handling, and the separate product export. Signals remains manual-only. Google Calendar still requires permission reauthorization; MailerLite two-factor sign-in is complete, its sending domain and lifecycle webhook are active, and the daily protected observer verifies four synced weekly preferences with an exact owner-private Sites receipt. The alert workflow is prepared but cannot activate with zero independently opted-in recipients. Bing has not confirmed a post-submission recrawl. These are open dependencies, not completed connections.

September 5 newsletter follow-through: welcome, reusable weekly campaign/template, alert template, preference confirmation and unsubscribe presentation were reconciled in MailerLite. The service-only observation migration and daily 12:30 UTC Vercel job are live. Thursday 08:00 Halifax private issue preparation is active; weekly send remains reviewed and manual. Command Centre v17 loads the full retained visibility data after the authenticated shell, fixing a live CPU-limit failure without removing evidence or weakening owner access. Production provider receipt, browser verification and known exceptions are in the private September 5 newsletter review.
