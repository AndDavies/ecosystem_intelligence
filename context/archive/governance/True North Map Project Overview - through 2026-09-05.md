Historical snapshot; not an active operating contract. Current authority: [current document](../../governance/True%20North%20Map%20Project%20Overview.md).
...
## September 5 desktop visual implementation — approved production release

The approved desktop designs are implemented across the homepage, directory, organization dossier and Signals, with the shared system carried into other route families. The original ship image, North Ink `#242827`, plain white hero lettering, pill actions, unboxed logos, open report sections, compact discovery controls and blue underlined editorial links replace the prior rounded-card default. About keeps its original jet hero. Signals use real sourced imagery or an explicit logo/brand-cover fallback and retain complete editorial content with one desktop contents rail.

September 5 browser feedback is incorporated: stable map expansion panels and a more prominent lookup, a bounded newsletter composition, aligned mission/defence-need rows, topic-specific yellow icons and a four-edition Signals archive with grouped neutral/yellow filters, published lead imagery and pagination. The homepage assessment has an ink/yellow treatment while retaining its source facts and unknowns. A short mist transition preserves the 50/50 ship hero and respects reduced motion.

Follow-up visual refinements restore rounded input fields and uniform map-search outlines, remove the hero letter stroke and redundant Defence needs disclosure, extend public footers across the viewport, and keep selected topic pills fixed in size without check icons.

Latest review refinements remove the hero highlight, use only a thin grey-to-North-Ink border change for text-field focus, reserve fixed map-table columns through expansion, and share topic-specific yellow icon medallions across mission/Defence need cards and their related-record bridges. Public organization-type controls now group accelerators, incubators, ecosystem organizations and government innovation offices under **Innovation & business support**. Directory, map, guided lenses and export matching share the grouping; detailed profile types and exact legacy type/category URLs remain unchanged. A September 5 read-only production check confirmed COVE is a published incubator in Dartmouth, Nova Scotia. The new group finds it under Atlantic without editing its classification.

Andrew approved committing and pushing the reviewed design release after removing the uppercase career tagline from About; the career paragraph remains. Node 24 release validation passed, including 829 tests, lint, typecheck, dependency audit, scale validation and production build. Brand System remains the detailed visual authority. Search, map/list state, Shortlists, forms, consent, authentication, source semantics, publication, data and providers remain governed by their existing contracts. No migration or provider-configuration write is needed. Signal sharing metadata now always supplies the published hero, approved presentation image or edition-specific branded fallback. The exact deployed commit and readiness are verified through GitHub, Vercel and the bounded post-ready launch gate; this source document does not substitute for that live deployment proof. No exhaustive production crawl is part of this release.
...
- The allowlisted relationship-presentation release candidate changes only how existing reviewed connections are sequenced and explained on `arctic-domain-awareness`, `persistent-uncrewed-underwater-surveillance` and `major-event-and-critical-infrastructure-cyber-defence`, with one Mission area and two Defence need controls retaining their existing presentation. Its deterministic view-layer helper exposes no score or ranking, leaves canonical relationships and global map order unchanged, and adds no schema, migration, provider, research, review or publication authority. Production acceptance remains conditional on the exact pushed SHA, a Vercel READY production deployment, bounded validation of all six cohort routes and a healthy `/api/health` response.
- The shared Paper-on-Field organization dossier is the approved canonical template for every organization kind. It uses a compact rendered-section index, contextual Mission area and Defence need relationships, bounded capability rows, source ledger, Shortlist and introduction actions, and approved-logo/monogram/neutral identity fallback. The eight reviewed pilot organizations and seven reviewed first-wave organizations now use it in production. Rollout remains record-specific: `editorial_profile_version = organization_editorial_profile_v1` activates one reviewed record, while null keeps its legacy profile. The deployed contract advertises v3/v2 support, but research staging, human acceptance, separate Publish and post-publish verification remain mandatory for each organization.
- The internal-link presentation graph is assembled deterministically from already published records. Organizations remain the principal destination; ownership and reviewed canonical relationships outrank Mission area, Defence need, program and editorial paths, while shared-mission or shared-domain similarity is labelled **Similar areas of work** and never presented as partnership or endorsement. The shared continuation module is bounded to three organizations, three mission/ecosystem paths and two Signals or Briefs. Inline editorial links remain North Ink with a persistent underline and Signal Yellow keyboard focus. Eligible database edges are measured by the private `links:inventory` report; rendered anchors, roles, contextual degree and click depth remain part of the explicitly authorized launch audit.
- Region cluster cards open the existing map through canonicalized `cluster=<slug>` state. Cluster filtering reuses reviewed capability-cluster membership, fails closed for unknown clusters and never substitutes an unrelated owned capability in the selected-record presentation. Program participation routes only to filtered map discovery. An indexable program page remains gated on an official source, useful summary, at least three canonical organizations and at least two verified participant companies; operator-only or otherwise thin records do not create public pages or sitemap entries.
...
- Organizations, Mission areas, Defence needs, Signals and North Signal now use
  ...
  shared neutral mark as fallbacks. Mission areas and Defence needs expose one
...
- Functional discovery collections share one decision-led sequence: concise
  ...
  Organizations, Regions, Mission areas, and Defence needs use whole-card
...
- Detail routes share one decision-handoff sequence: understand the published
  ...
  Mission area or Defence need relationships, and carry selected organizations,
  capabilities and evidence into a private Shortlist. Dossier links preserve
  safe map-return context, while region, mission, and Defence need continuations
...
- The supporting public journey now uses the same outcome-led language as the
  ...
  capability, evidence, a private Shortlist, and a next conversation through
...
- Every published organization route now targets the same evidence-bounded
  ...
  evidence of maturity, reviewed Mission area and Defence need contribution,
