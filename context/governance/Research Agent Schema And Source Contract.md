...
The deployed executable research pipeline is `tnm-research-pipeline/1.7.1`. Corpus-scale dossier enrichment uses its normalized outputs: `organization_bundle_v3` for new organizations and `organization_refresh_bundle_v2` for existing records. Local files alone never make those shapes stageable; the deployed application must advertise the compatible pipeline patch plus Review and Publish support before private intake.
