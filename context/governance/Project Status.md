...
- Andrew completed the overnight dossier-review and publication sequence on August 12. Exact current corpus, Review and Publication totals must be read from production rather than reconstructed from the earlier wave narrative. Research remains review-first: subsequent discovery and enrichment runs may add validated private candidates, but only Andrew's later Review and Publish actions activate records.
...
- The private visibility workflow remains available for Andrew to invoke, while its Monday 08:00 America/Halifax automation is currently paused. Its local contract now separates Search Console's dimension-free headline totals from public page-only opportunities and privacy-thresholded query attribution; classifies GA4 AI referrals only through a known-host allowlist; requests DataForSEO asynchronous AI Overview detail while keeping unresolved citation presence unknown; and reports configured-live completeness separately from whole-portfolio coverage. When run, it validates its local contract, preflights configuration/authentication, queries every configured read-only provider with paginated responses and a complete public-sitemap audit, and synchronizes only the allowlisted owner dashboard summary. Unconfigured Bing and dedicated Generative AI APIs remain explicitly unavailable/unknown and do not fail strict mode; a configured provider failure still does. It does not check credits, cap DataForSEO tasks, reuse same-day panels, or change billing. Incomplete configured-provider evidence is a failed monitoring run, not a successful zero-data report.
- The enriched owner-only Visibility Lab is deployed with in-place workflow/public-page loading and sanitized search, route-family, device, geography, referral, technical, PageSpeed and AEO diagnostics. Its August 11 weekly snapshot was acknowledged exactly at `2026-08-11T23:23:44.409Z` with no outbox item. The August 11 strict refresh remains incomplete despite current configured-provider evidence because four public Brief routes returned HTTP 500 during the complete 875-route audit; the dashboard therefore remains a monitoring surface rather than proof of a successful strict run.
- The enriched owner-dashboard contract adds sanitized search-completeness and momentum diagnostics, technical eligibility, route-family and opportunity distributions, closed-category question demand, seed-intent and AEO feature coverage, aggregate AI-referral history, insights, and actions without exposing raw queries, referrers, provider payloads, exports, visitor data, or private routes. Its Current Workflow surface reveals more items in place with Load more rather than navigating between pages. This contract change does not itself prove a completed provider refresh or dashboard acknowledgement.
- Search Console bulk export is active for the verified `https://truenorthmap.ca/` property in the owner-controlled Google Cloud project, writing to the Montréal `searchconsole_truenorthmap` dataset. The export service and private collector have least-privilege BigQuery roles. The collector uses aggregate export-health, day/row coverage, headline, anonymization, and position evidence while raw exported query text remains local. Google allows up to 48 hours for first-table creation, so that dated warm-up state is reported as pending and does not fail strict refreshes; once the window expires, a failed configured BigQuery query is blocking.
...
- The North Signal editorial skill reads published Signals and their original durable sources, scans the validated 38-source register whose 37 active feeds are mirrored in the OPML and live Inoreader account, treats selected Gmail newsletters and search as discovery only, produces `north_signal_issue_v2`, validates its six ordered sections, 700-1,100-word envelope, issue-window dates and product links, records per-run feed health, and stops for Andrew's editorial review. It never creates or sends a MailerLite campaign automatically. The August 12 health probe resolved 36 registered feeds as available, retained the stale NRC feed with a direct official-news fallback, quarantined Vanguard's malformed XML outside the active OPML, and left zero unresolved rows. Ten new official feeds are on 30-day probation in `Canada - Official`.
- The North Signal acquisition application and bounded event-name migration are deployed. `/north-signal`, the Signals-led popup and contextual forms, acquisition telemetry, Admin Insights reporting and `/signals/feed.xml` are live; the exact running application commit remains visible through the deployed research-contract endpoint and release evidence rather than a hand-maintained status sentence. A controlled production signup previously proved landing attribution, the Supabase consent write, MailerLite synchronization, authenticated Gmail delivery, both destination links, the lawful footer, unsubscribe headers, SPF, DKIM and DMARC. Disposable test contacts, consent rows and test-session events were removed afterward, restoring the consent-backed production state; no outreach message, social post, paid promotion or weekly campaign has been sent.
- The MailerLite welcome and weekly draft now share the documented True North Map email system while retaining different jobs. On August 12 the existing single-message welcome was rebuilt with a compact Directional N, `NORTH SIGNAL · WEEKLY` label, five-minute orientation, one Signal Yellow Signals action and one Mission Areas link. Desktop and mobile provider previews passed; it was reactivated for new `Ecosystem Intelligence` entrants only, and its four completed and zero in-progress counts were unchanged, proving no existing member was retriggered. The weekly campaign draft now uses the branded Signals masthead, one-to-three real Signal structure, original-source treatment, bounded assessment, explicit unknowns and one tracked `/signals` action. Desktop and mobile provider previews passed after the mobile masthead was compacted. It has no recipients, schedule, Outbox item or send. MailerLite's standalone `My templates` editor previously returned a repeatable 503, but the campaign editor and saved draft are usable. Gmail and non-Gmail inbox tests, per-issue content and link completion, dedicated-group selection and Andrew's explicit authorization remain required.
...
2. Read the live Review and Publication queues before each research or publication task. Continue non-overlapping enrichment segments only when requested; run new-organization discovery against both the published corpus and every active candidate, and keep every resulting record private until Andrew reviews and separately publishes it. Respond to real connection, contact, contribution and feedback items through their ordinary workflows.
