...
- Production now carries the ordered dossier schema and application contract `tnm-review-publication-v3`, but the first low-rate crawl of application commit `aa6dc34` exposed a P1 loader defect: null-version legacy profiles still queried the correlated rich dossier view before the version gate and created bounded REST 500s and statement timeouts. A scoped release hotfix now resolves the identity/version first and reserves the rich view for exact `organization_editorial_profile_v1` records. Broader promotion and enrichment remain held until the hotfix passes its clean production crawl and log gate. Zero profiles are activated, so every current organization continues to use the legacy public presentation until a reviewed enrichment candidate is separately accepted and published.
