...
- On 2026-08-10, Andrew reviewed, accepted and separately published all eight candidates from `tnm-dossier-pilot-20260809`. Production consequently has eight activated organization dossiers. The pilot established the accepted shared layout, content hierarchy and action model across company and research/test-centre records. Pipeline `tnm-research-pipeline/1.7.1` is now deployed, and exactly one seven-target corpus run, `tnm-dossier-corpus-wave1-20260810`, has been imported to private Admin Review. Its seven moderate-confidence refresh candidates are pending with zero review decisions, approvals, publications or target activations. The run uses exact named targets, at least three complementary searched lanes per target, complete twelve-dimension coverage, durable corroboration for consequential claims, explicit unknowns, and marginal-yield stopping without a numeric article or source quota. Andrew retains every acceptance and separate Publish decision.
...
- The deployed `tnm-review-publication-v3` interoperability contract supports `organization_bundle_v3` for new normalized dossiers plus `organization_refresh_bundle_v2` for cited additive or in-place enrichment under pipeline 1.7.1. The deployed `/api/system/research-contract` advertises that compatible pipeline patch, and the first seven-target corpus wave passed its trusted-import gate. Both candidate shapes remain guarded by the deployment check, private Admin Review, a separate Publish checkpoint, stale-baseline protection, and exact per-leaf evidence. The current intake staged seven pending candidates but activated no organization profile.
...
2. Review the seven pending candidates from `tnm-dossier-corpus-wave1-20260810` before preparing another corpus batch. Work only records Andrew accepts through the separate Publish checkpoint after reviewing operations, evidence and amber warnings; respond to real connection, contact, contribution and feedback items through their ordinary workflows.
