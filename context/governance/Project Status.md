...
- A later seven-target organization run remains private in Admin Review because its size came from the old named-wave configuration, not from evidence scarcity. The first production `corpus_refresh` segment has now added 50 further non-overlapping private candidates under pipeline 1.7.2. Production therefore has 57 pending organization refreshes and zero approved candidates at the latest reconciliation. The new segment carries 131 selected durable sources and 224 atomic claims across 50 role-balanced organizations; only three candidates propose Recent Activity because the other 47 had no durable dated development. All 57 records remain under Andrew's normal Review and separate Publish authority.
...
- North Signal is the named weekly decision brief. The local acquisition release adds `/north-signal` as the campaign hub and keeps `/signals` as the public sample library; contextual forms appear after useful content on Signals, Mission Areas, Public Needs, organization profiles and the homepage. The desktop prompt waits for high-intent engagement, mobile uses a compact banner and bottom sheet, and both reuse the approved grayscale fighter/map artwork with Signal Yellow afterburners. The journey respects a 30-day dismissal, remains available from the header and footer, records one-action affirmative consent before MailerLite synchronization, and reports landing views, sample clicks, form starts, submits, successful consent writes, errors and dismissals separately from the live subscriber total.
...
- The North Signal acquisition application and bounded event-name migration are deployed at commit `459cc32`. `/north-signal`, the Signals-led popup and contextual forms, acquisition telemetry, Admin Insights reporting and `/signals/feed.xml` are live. One production signup test completed the Supabase consent write, MailerLite subscriber synchronization and authenticated Gmail welcome delivery. The live MailerLite welcome content still uses the superseded Defence Brief CTA and remains an explicit provider-side correction before final acquisition closure; no campaign, outreach message, social post or paid promotion has been sent.
...
2. Review the 57 pending organization refresh candidates: the earlier seven-record packet and the first 50-record production `corpus_refresh` segment. Continue automatically selected non-overlapping production segments until every eligible target receives a candidate or typed disposition; only records Andrew accepts through the separate selectable Publish checkpoint become activated. Respond to real connection, contact, contribution and feedback items through their ordinary workflows.
