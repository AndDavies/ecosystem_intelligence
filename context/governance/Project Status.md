...
- A later seven-target organization run is currently private in Admin Review: live verification found seven pending refresh candidates, zero approved candidates, and no queued or running research run. Its size came from the old named-wave configuration, not from evidence scarcity. Those pending records remain under Andrew's normal Review and separate Publish authority; pipeline 1.7.2 does not rewrite their staged payloads.
...
- The North Signal acquisition application and bounded event-name migration are local and unapplied. Production remains on the existing acquisition surface until Andrew approves migration and release; no MailerLite template, welcome automation, subscriber group, campaign, outreach message or paid promotion was changed by the implementation.
