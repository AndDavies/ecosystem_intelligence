...
- Production carries the ordered dossier schema, application contract `tnm-review-publication-v3`, and verified read-path hotfix `8a29b13`. The shared loader now resolves the published identity/version first, reserves the rich view for exact `organization_editorial_profile_v1` records, and uses the bounded legacy path for every current unversioned record. All affected profiles, the 869-page crawl, health/count reconciliation, GitHub/Vercel and PostgreSQL log gates passed without a new timeout. Zero profiles are activated, so every current organization continues to use the legacy public presentation. Reviewed source enrichment may now begin in bounded private batches, but the first separate Publish still requires the representative rich/sparse/non-company/mobile/keyboard matrix and a cold-cache versioned-route/PDF check.
- On 2026-08-10, the named eight-organization dossier pilot was regenerated under `tnm-research-pipeline/1.7.0` and replaced in the existing private queue through the guarded same-ID pending-only path. The final frozen export was imported as eight staged and zero skipped; all eight exact baselines still match production, and the rows remain pending with zero review decisions, acceptance, publication, or profile activation. Commit `7fbcc9f` deploys the complete same-run record-specific gate and repairs Admin Review so field changes and the editable reviewer decision are readable without duplicated generated rationale. The exact production deployment, authenticated Review queue, GitHub checks, health and catalogue reads, 870-page zero-warning crawl, and Vercel/PostgreSQL log window passed. These remain private review and application changes only; Andrew still decides every acceptance and separate Publish action.
...
Every new OSINT-enabled run writes a private `research_collection_plan_v1` and `research_claim_ledger_v1`. The ledger is run lineage, not another database or review queue. It stores atomic claims, canonical URLs, locators, temporal scope, source independence, contradictions, supersession, candidate field targets, and a twelve-dimension dossier coverage vector. The coordinator searches entity-outward and problem-inward, decomposes independently reviewable capabilities, pairs capability evidence with official Mission Area or Public Need premises, classifies consequential unknowns, and requires each rationale to expose coverage value, evidence, the conservative Mission/Public Need read, unknowns, and one bounded reviewer action. Pipeline `tnm-research-pipeline/1.7.0` retains the 1.5/1.6 typed dossier and atomic-leaf contracts while adding a complete same-run specificity gate: every named target has one candidate or structured disposition; prospect, signal, lead, operation, claim, recovery, and rationale text uses structured record anchors; every source-backed leaf and coverage subject has one eligible owner; refresh leads and signals match the candidate target, baseline and typed outcome; and trusted import deep-compares the validated candidate batch with the staging payload. Historical 1.5 and 1.6 artifacts remain immutable under their recorded versions. Exact Public Need hypotheses remain private Derived Reads until a published capability enters the existing demand-matching workflow. The deployment-compatibility preflight still checks the canonical production contract rather than inheriting a local browser URL, so a stopped localhost server cannot create a false staging failure.

## Current tracked lineage posture

The August 5 reconciliation retained completed published research lineage, reusable visibility tooling and no raw provider material. The August 10 dossier pilot adds one complete, validated 1.7 lineage set for eight named organizations. Its matching production candidates are still private and pending; the tracked files record the final corrective staging inputs and do not authorize or mirror a later human decision. Raw source-working material and the deterministic local builder remain ignored. Releases continue to use explicit path staging and never a blanket stage command.

Files under `research/ingestion/runs/`, `reviews-v2/`, `staging/`, and candidate/lead directories are point-in-time provenance. After the final corrective restage they remain unchanged and therefore do not mirror later review decisions. Exact live queue totals must still be read from production. Production Supabase remains the only current queue and corpus authority. The deployed application now carries pipeline 1.7 and the Admin Review repair at commit `7fbcc9f`; that deployment does not accept, publish, activate, or make the tracked lineage a runtime queue.
