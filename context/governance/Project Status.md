...
- On 2026-08-10, the named eight-organization dossier pilot was regenerated under `tnm-research-pipeline/1.7.0` and replaced in the existing private queue through the guarded same-ID pending-only path. The final frozen export was imported as eight staged and zero skipped; all eight exact baselines still match production, and the rows remain pending with zero review decisions, acceptance, publication, or profile activation. The release candidate also repairs Admin Review so field changes and the editable reviewer decision are readable without duplicated generated rationale. These are private review and application changes only; Andrew still decides every acceptance and separate Publish action.
