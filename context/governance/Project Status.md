...
- Canadian Defence Signals as an isolated weekday automated editorial surface. Descriptive immutable URLs, source links, visible uncertainty, correction timestamps, RLS, admin archive/correction tools, run health, and private social drafts are live. Automatic authority is limited to dedicated `signal_*` tables, and the executive field-guide editorial gate must pass before publication.
...
4. Monitor the first live Canadian Defence Signals edition and decide separately whether to activate a weekday schedule. Defence Brief and weekly North Signal remain human-reviewed editorial products; social drafts remain private until explicitly used.
