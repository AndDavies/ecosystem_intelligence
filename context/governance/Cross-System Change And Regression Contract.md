...
| Canadian Defence Signals | Exact-eight v2 item count, distinct primary durable pages, honest source-family count, typed no-publish audit, credential-verified historical v1 repair, fact/read separation, unknowns, duplicate fingerprint, immutable descriptive slug, stable RSS GUID/timestamp, mandatory private LinkedIn and X examples, no core-corpus write | Overview, status, Daily Signals skill and route contract | V2 accepts exactly eight and rejects seven, nine, duplicate primary pages and false family counts; no-publish is payload-hash idempotent and creates one private run row with zero edition/item/source/media/social/alert writes; v1 cannot create a new edition and dry-run succeeds only for an exact existing run/slug/date repair; migration/RLS chain, dry-run and idempotent apply, current-edition social-example verification in Admin, metadata, complete sitemap/archive, RSS no-backlog/no-duplicate/correction behavior, admin correction/archive, 390/768/1024/1440 reading layout |
