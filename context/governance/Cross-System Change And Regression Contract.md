...
| Internal-link graph, continuation module, or filtered-map route | Organization-first destination priority, reviewed relationship wording, native anchors, contextual degree, click depth, compact map payload, analytics privacy and reciprocal route coverage | Overview, Brand System, Research Contract when linkability changes, Skills Map and Development Log | `pnpm links:inventory`, graph and route tests, lint, responsive and keyboard checks at 390/768/1024/1440; `pnpm scale:validate` when map projection/filtering changes; the database inventory measures eligible reviewed edges while only an explicitly authorized `pnpm launch:audit` proves rendered-site coverage |
...
Public discovery-cache changes must prove that every published organization remains in the compact map/search projection and that no individual cache item grows beyond its provider limit. After the canonical transaction succeeds, Publish invalidates the shared discovery tag, cacheable public indexes and affected record paths once; it must not synchronously re-read or warm the national corpus and must not trigger speculative dossier prefetch. `/organizations`, `/missions` and `/regions` use publication-driven freshness with a one-day expiry only as an out-of-band recovery backstop. Query-bearing organization-directory requests retain their browser-visible URL and use the isolated dynamic filter renderer. Organization and capability dossiers remain dynamically rendered while request-specific return and release-probe state is coupled to their page response, but their published data caches are tag-invalidated by successful writes. The owner-only manual atlas revalidation endpoint remains the explicit recovery flush.
