...
| Organization, capability, region, mission, map, or search | Compact national projection, deterministic Data API paging, pagination, filters, URL state, API shape, Ask True North eligibility, exports, public profile allowlists, bounded citation hydration and dossier-family parity | Overview and relevant data/admin contract | Tests, lint, 5,000-marker scale gate where map loading changes, public summary/atlas count comparison, API and route smoke; for dossier-view/citation changes add the exact-deployment cold-dossier gate across high-citation, sparse, recently updated and coverage-fill records; verify first-viewport map geometry, the 380-pixel desktop rail, mobile sheet states, Map/List control, and accessible table when `/map` changes |
| Supabase schema, RLS, storage, or server action | Anonymous/member/admin matrix, migrations, review and publish, caching and revalidation, private-data exposure, public JSON allowlists, irreversible cleanup and application/schema order | Admin/Data Contract, Status, migration notes | Migration/RLS tests, release validation, exact repository/live-ledger comparison, application-before-cleanup proof, public-response forbidden-key scan, security advisors, production smoke |
| Research schema or skill | All downstream skills, executable pipeline schema, deployed research contract, Admin Review and Publish support, public assessment citations | AGENTS, pipeline docs, skill references, status | Data readiness, research validation, smoke/staging compatibility, review-card inspection; a pipeline increment cannot stage until the deployed schema, Review and Publish paths all advertise support |
...
| Analytics, newsletter, feedback, or contact | Consent, private-route exclusion, masking, retention, raw-ledger preservation, scorecard cohort filtering, MailerLite sync, webhooks | Privacy, status, runbook | Consent states, focused-flow suppression, event-name migration ordering, CTA-to-success funnel, QA/staff/test exclusion from scorecards without raw-event deletion, sync/webhook, scheduled retention |
| SEO, AEO, structured data, or sharing | Canonicals, sitemap freshness, robots, normalized internal links and referrers, explicitly marked durable outbound sources, social image crops, visible-source claims | Visibility skill/report contract and brand system | Visibility validation, bounded post-deploy launch gate, structured-data and share preview checks; add the paced full launch audit only for major sitemap/internal-link architecture changes, scheduled assurance or an explicit broad audit; confirmed broken marked sources block while bot-restricted and transport-unknown results are reported separately |
| Loading, empty, error, permission, or not-found state | Route geometry, recovery links, private-data boundaries, robots, keyboard focus, and screen-reader announcements | Overview, route contract, and development log when shared language changes | Scoped state tests, one-H1 and overflow checks, affected routes at 390/768/1024/1440, clean build, and bounded post-deploy route validation |
...
| North Signal acquisition or issue contract | Affirmative consent, MailerLite sync isolation, event-name migration, bounded UTM attribution, raw-ledger/scorecard separation, `/north-signal` metadata, `/signals` proof links, RSS summaries, preserved Brief archive, 28-feed health, original-source lineage, private review/send authority | Overview, status, Email Updates Operations, brand system, North Signal skill and tracked email source | Consent idempotency and provider-failure tests; CTA click, form, submit and success funnel plus historical-placement reporting; QA/staff exclusion from the private scorecard; landing/popup at 390/768/1024/1440 plus keyboard/focus/zoom checks; RSS validation; v2 issue accepts the complete fixture and rejects missing Signal links, unknown/discovery-only sources, duplicate sections and Brief links; provider presentation and links are tested manually without implying send authority |
...
`visibility:validate` is a separate local operator check and applies only when private visibility work changes. `launch:validate` is a bounded live-origin gate and is authoritative only after the exact candidate commit is deployed; it must not be used before push to claim that uncommitted code is live. Before push, use route tests, the required browser matrix and `release:validate`. After Vercel is ready, `launch:validate` checks the exact deployed commit, operational consistency, sitemap and representative/affected public routes in roughly 15–20 requests. An RSC error digest, an unresolved streamed or route loading shell, or a dynamic-metadata failure is an operational blocker even when the HTTP status is 200. One recovered retry is advisory and requires a direct recheck plus log review; two recovered routes, strict-mode recovery or a live error cluster block closure. `launch:audit` is the serialized, paced, health-aware full-site inventory and runs only for a major information-architecture/publication change, scheduled assurance, or explicit broad-launch audit. It normalizes discovered same-origin navigation targets, strips fragments and acquisition-only attribution, records referrers and redirects, and visits each resulting target once within the configured ceiling. Its outbound phase probes only links deliberately marked as durable public sources; private or reserved hosts are rejected, ordinary provider/social/campaign links are excluded, and healthy, redirected, confirmed-broken, bot-restricted and transport-unknown results remain distinct. Only confirmed broken sources block. It never belongs in push CI and never becomes a load test. `scale:validate` uses a deterministic 5,000-marker fixture to verify complete projection, bounded rich results, serialized-size budget, and fallback-clustering responsiveness without touching production.
