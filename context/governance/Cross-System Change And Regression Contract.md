...
| SEO, AEO, structured data, or sharing | Canonicals, sitemap freshness, robots, internal links, social image crops, visible-source claims | Visibility skill/report contract and brand system | Visibility validation, bounded post-deploy launch gate, structured-data and share preview checks; add the full launch audit only for major sitemap/internal-link architecture changes or scheduled assurance |
...
| North Signal acquisition, issue, or provider presentation | Affirmative consent, MailerLite sync isolation, event-name migration, bounded UTM attribution, `/north-signal` metadata, `/signals` proof links, RSS summaries, preserved Brief archive, 38-source/37-active feed health, original-source lineage, tracked source/OPML/live-provider parity, brand assets and alt text, welcome trigger scope, private review/send authority | Overview, status, Email Updates Operations, Email and Domain Infrastructure, brand system, North Signal skill, welcome and weekly source templates | Consent idempotency and provider-failure tests; funnel and historical-placement reporting; landing/popup at 390/768/1024/1440 plus keyboard/focus/zoom checks; RSS validation; v2 issue accepts the complete fixture and rejects wrong section order, content outside 700-1,100 words, out-of-window dated sources, missing Signal links, unknown/discovery-only sources, duplicate sections and Brief links; registry/OPML/provider parity; MailerLite desktop/mobile preview; Gmail and non-Gmail inbox tests before weekly send; sender, reply-to, dedicated group, lawful footer, unsubscribe and every destination link; activation leaves existing completion/in-progress counts unchanged; campaign remains without recipients, schedule or send until Andrew authorizes it |
...
`visibility:validate` is a separate local operator check and applies only when private visibility work changes. `launch:validate` is a bounded live-origin gate and is authoritative only after the exact candidate commit is deployed; it must not be used before push to claim that uncommitted code is live. Before push, use route tests, the required browser matrix and `release:validate`. After Vercel is ready, `launch:validate` checks the exact deployed commit, operational consistency, sitemap and representative/affected public routes in roughly 15–20 requests. One recovered retry is advisory and requires a direct recheck plus log review; two recovered routes, strict-mode recovery or a live error cluster block closure. `launch:audit` is the serialized, paced, health-aware full-sitemap inventory and runs only for a major information-architecture/publication change, scheduled assurance, or explicit broad-launch audit. It never belongs in push CI. `scale:validate` uses a deterministic 5,000-marker fixture to verify complete projection, bounded rich results, serialized-size budget, and fallback-clustering responsiveness without touching production.
