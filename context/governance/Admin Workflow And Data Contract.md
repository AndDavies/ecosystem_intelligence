...
Signals text fields provide bold, italic, HTTP(S)/internal links, bullet/numbered lists and an unsaved preview. A small Markdown-style vocabulary is stored in existing text fields; titles remain plain text. Preview and public narrative share the renderer and `atlas-prose-link` styling. Raw HTML is escaped and unsupported URL schemes are not made clickable. The link dialog offers New tab (default for new links) and Same tab. New-tab links store a `{target=_blank}` suffix and render with `noopener noreferrer`; Same tab stores `{target=_self}`. Both suffixes are omitted from excerpts. HTTP(S) links without a suffix, including existing and pasted links, default to a new tab; relative internal links without a suffix stay in the same tab. Explicit choices override either default and survive edition/item saves. Added inline links are editorial links, not automatic additions to immutable original-source evidence. Existing admin-only save, amendment/audit and cache invalidation controls apply; saving a published edition updates its public text.
...
The separate Publish action locks the candidate and organization, uses the immutable `before_record` timestamp as the authoritative parent baseline, and fails atomically if the target changed after research. Its approved queue is also grouped by research run rather than silently showing only the first 50 approvals. The checkpoint lets the reviewer choose one run and select any subset of its displayed approved records; only that selected set enters the all-or-nothing transaction, so one deferred or suspect approval does not force publication or block unrelated ready records. For every `update_child`, the transaction locks the owned child, reconstructs the complete normalized public snapshot, and compares it with the schema-valid reviewed `before` payload before applying any change. Candidate creation, smoke validation, trusted staging, and database triggers reject timestamp precision loss and a child baseline that does not equal the current published capability, domain/Mission relations, program participation, organization relationship, or funding event before review. The publication transaction repeats the child comparison to cover a legitimate canonical change after staging. Direct owner child corrections acquire the same parent-first lock and advance the parent review timestamp, so an already-staged refresh becomes stale instead of overwriting the correction. A genuinely stale parent or child produces a candidate-specific administrator message and requires a fresh candidate plus human review; for a single blocked candidate the checkpoint offers an explicit return-to-research action rather than implying its checkbox was ignored. An eligible publication applies only the reviewed operations, preserves existing IDs and slugs, upserts sources, appends evidence snippets and field citations using leaf-local targets, records the before/operation audit payload, and only after that transaction succeeds invalidates the shared discovery cache, cacheable public indexes, sitemap and affected dossier paths. Failed or rejected publication performs no public cache invalidation.

Program participation candidates that reference an existing canonical program must carry that program's current published name, type, operator, URL and summary exactly. A conflicting same-slug payload is rejected before Admin Review and remains fail-closed at Publish; research must reuse the canonical program definition while keeping the new organization-specific participation and its evidence distinct.
