...
- The August 10 research-review integrity release is active and its governance closure is deployed at commit `a3c52f8` / deployment `dpl_DDKUbd8B8GamWuA7c6aBeXrx2ZsH`. It advances new research runs to pipeline 1.7, enforces complete record-specific lineage and exact trusted-import parity, and repairs Admin Review presentation without a database migration. The Node 24 release gate, GitHub Release Validation, CodeQL, authenticated Admin Review, health/count reconciliation, 870-page zero-warning crawl and Vercel/PostgreSQL log window passed. Andrew subsequently reviewed, accepted and separately published all eight pilot candidates; production now has eight activated dossier profiles and zero pending or approved candidates from that run. The exact-eight Daily Signals v2 plus typed no-publish writer and the current 1.7.1 operator/Admin refinements remain local changes and are not production authority until a later verified deployment. The 06:30 automation is paused to avoid its historical v1 prompt conflicting with the local v2 skill. Release closure must prove a no-publish apply creates one private run row and zero edition/item/source/media/social rows, a v1 dry-run requires an exact existing run, and the automation receives the v2 prompt and is reactivated only after the compatible deployment is live.
...
- The August 9 organization-dossier foundation remains the version-gated application basis. Both ordered migrations are recorded, the deployed contract advertises `tnm-review-publication-v3`, and the health, policy, route, count, advisor, GitHub, Vercel, Postgres-log and crawl gates passed. The eight owner-reviewed pilot records now prove the shared template in production; the remaining 407 published organizations stay on the bounded legacy path until individually enriched, reviewed and published. Never replace that controlled rollout with a global version update.
