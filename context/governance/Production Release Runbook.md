...
| `pnpm launch:audit` | Only through explicit `$tnm-site-assurance` for major sitemap/internal-link architecture change, scheduled assurance, explicit broad audit, or systemic diagnosis | Serialized normalized same-origin navigation crawl with referrers/redirects, plus supporting pagination, duplicate-title/orphan/performance inventory and a capped probe of deliberately marked durable outbound sources. Production refuses to start without `PUBLIC_LAUNCH_FULL_AUDIT_ACK` and an approved `PUBLIC_LAUNCH_AUDIT_REASON`. | Internal operational/HTTP failures and confirmed broken marked sources are blockers. Redirects, bot restrictions and transport uncertainty remain separate report classes. SEO/link/performance inventory remains a reported remediation list rather than making an unrelated application change disappear. Lock-held or circuit-breaker exit means inconclusive, not failed product code. |
