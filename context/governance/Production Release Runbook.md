...
## September 5 baseline migration order

The local baseline candidate contains `20260905161114_harden_member_workflow_boundaries.sql` and `20260905162407_reserve_assistant_requests.sql`. Before applying either, recheck existing Working List text lengths, the exact migration ledger and `true-north-map-purge-expired-product-telemetry`. Both migrations are additive and compatible with the existing application; apply them in filename order before deploying the application that calls `reserve_assistant_request`. Without the reservation RPC, the new application deliberately falls back instead of invoking the paid provider.

The second migration extends the existing private retention function without changing its result shape or adding a scheduler. Recheck service-only RPC grants, member/anonymous denials, direct workflow predicates, retention ownership and the scheduled job after applying. Rolling back application code does not require dropping the security constraints or reservation ledger; retain them and use a reviewed forward repair if needed. Run the full Node 24 release gate, focused database/route regressions and local browser checks before the release decision, then the bounded exact-deployment checks after push. No candidate acceptance, publication, campaign or provider write is implied by this release.
...
Run scoped v3/historical/failure tests and the ordinary release gates. Verify immutable evidence, text-led and source-image previews, Admin correction/archive, stable RSS identity, nonpublishing isolation and unchanged core records. Install the private skill only after the exact deployment is ready. Daily Signals runs manually from Andrew's chat; its automation and scheduling contract have been deleted. Today's first current-source edition is a separate requested run after implementation.
