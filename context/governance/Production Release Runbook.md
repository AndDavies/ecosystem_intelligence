...
- Application commit `aa6dc34` and Vercel deployment `dpl_Av7PCX7qau7sJqtnvpMNYU6osLVQ` successfully advanced the deployed contract to v3, but the first paced crawl triggered rich-view timeouts for unactivated legacy profiles. Release closure and enrichment are held while the version-first shared-loader hotfix completes the full local and production gate. If route availability or database pressure deteriorates before that repair is verified, promote the last verified application deployment `783b7f5` / `dpl_EPn1AWT5cpG45aof4YBobbkaLLpc` and preserve the forward-compatible dossier schema for repair-forward.
