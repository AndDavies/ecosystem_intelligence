...
| REL-2026-001 | A capability route produced a transient production 500 while loading public citations | Every dossier loaded all public field citations, evidence snippets, and sources even when the requested organization or technology was already scoped | Build citation targets from the selected organizations, technologies, reviewed matches, funding events, and public needs; query indexed entity batches; then load only the referenced approved evidence and sources | Targeted loader tests, repeated affected-route requests, full canonical crawl, clean build, and production-log review with no citation statement timeout | Resolved in source; production verification pending |
