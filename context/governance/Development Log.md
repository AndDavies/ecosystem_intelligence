...
Launch assurance is now split at the source. `pnpm launch:validate` is a bounded post-deployment gate that checks the exact deployed commit, operational consistency, sitemap/RSS/latest-Signals proof, five critical routes, one representative per public record family and at most ten explicit affected paths. It refuses cross-origin redirects, permits one advisory recovered route and blocks repeated recovery. `pnpm launch:audit` owns the exhaustive inventory only when a major sitemap/internal-link change, scheduled assurance or explicit broad-launch audit justifies it; the audit is serialized at a 750 ms minimum plus jitter, cross-chat locked with live-owner and heartbeat checks, identified, progressively reported, health-checked every 25 pages, circuit-broken on repeated final or recovered pressure, and separates HTTP/operational blockers from SEO/orphan/duplicate/performance follow-up. An interrupt records an inconclusive result instead of leaving a phantom running audit. The live site recovered after the earlier burst; fresh health and sampled routes returned normally and the final release still requires the bounded exact-deployment gate plus a clean post-ready log window. No subscriber, provider, campaign, research, atlas or publication authority changes are part of this UI update.
