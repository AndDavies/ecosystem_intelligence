...
It must never invent an organization or fact. Ask True North uses the OpenAI Responses API with an explicitly configured server model and no code fallback, strict structured output, low reasoning effort, no web tools, no saved model response, a server-validated known-ID boundary, and the canonical published Supabase snapshot. The model remains server-configurable through `OPENAI_MODEL`, but every replacement must preserve structured output and the same published-record guardrails. A deterministic baseline selects current published records. Optional Jev scores the full published catalogue before the 16-organization answer limit; requested constraints are interpreted once per question and continuous relevance remains primary. The authenticated owner-pilot may run uncapped fixed-question comparisons; public activation retains the five-second/$0.05 safeguards and separate evaluation gate. Owner comparisons retain failed attempts, actual answering evidence and human usefulness notes. Functional fit and stored source confidence remain distinct; decisive unsupported requirements are visible alongside adjacent suggestions. See [Ask Jev integration](../../app/docs/ask-jev.md). Anonymous visitors receive three questions per rolling 24 hours and signed-in users receive 20. Follow-up context is temporary browser state only. Quota, timeout, refusal, missing-key, dependency, authentication, API-quota, rate-limit, model-access, network, and invalid-output failures are recorded as sanitized operational classes and fall back to deterministic discovery rather than failing the map. Ambiguous questions return suggested filters.
...
Optional Jev selection evaluates the published catalogue before the 16-organization answering limit, with a five-second and US$0.05 input-cost ceiling in standard mode and complete deterministic fallback. The server-authenticated owner-pilot baseline removes those two application cutoffs to measure full-pass latency and actual cost; provider/host limits and the existing Ask request allowance still apply to the public Ask route. A server-verified owner-only test page provides manually initiated, fixed-question comparisons without consuming that visitor allowance. Judgments are internal selection hints, not evidence or eligibility. Default disabled; owner pilot and public activation follow the evaluation and provider-disclosure checks in [Ask Jev integration](../../app/docs/ask-jev.md). Deterministic map search is unchanged.
