...
| Map (`/map`) | Begin with Ask True North or browse a published Mission Area or Public Need in place; filter the visible map, compare the synchronized results rail or accessible list, and open dossiers | Published records only; selecting a starting record does not leave `/map` or consume an Ask True North question |
