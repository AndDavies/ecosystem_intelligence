...
The tracked executable interoperability surface now recognizes `organization_bundle_v3` and `organization_refresh_bundle_v2` under `tnm-research-pipeline/1.7.0`. The installed private research chain is aligned to the same complete-run, record-specific gate. Local skills must continue to fail closed until the deployed research contract advertises those candidate versions; deployment support does not grant staging, acceptance, publication, or template-activation authority.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| WF-01 | Workflow | Yes | `.agents/skills/tnm-autonomous-research/` | Andrew-invoked manual run | Production coverage, taxonomy, Source Book, durable sources | 1.7 record-specific run lineage, exact dossier-readiness dispositions, and pending Admin Review candidates | May call the guarded staging RPC only; never accepts or publishes | Complete same-run specificity gate, `pnpm research:validate`, and deployed research contract | Andrew reviews, edits, accepts, and separately publishes | Active manual | Andrew Davies | 2026-08-10 |
...
| ST-01 | Internal skill | No | `.agents/skills/tnm-signal-refresh/` | Explicit coordinator delegation | Watchlists, watermarks, bounded source families | Dispositioned atomic signals with mandatory record-specific change summaries | Local lineage only | Research smoke and 1.7 specificity validation | Review-first chain | Active explicit-only | Andrew Davies | 2026-08-10 |
...
| ST-06 | Internal skill | No | `.agents/skills/tnm-review-steward/` | Explicit coordinator delegation | Complete candidate batch and same-run lineage | Specificity-gated pending Admin Review intake with readable operation cards | Guarded staging RPC only | Deployed contract, exact-target, payload-parity, evidence, and review-card checks | Andrew accepts and separately publishes | Active explicit-only | Andrew Davies | 2026-08-10 |
