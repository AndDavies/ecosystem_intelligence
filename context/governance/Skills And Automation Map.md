...
The deployed executable interoperability surface recognizes `organization_bundle_v3` and `organization_refresh_bundle_v2` under `tnm-research-pipeline/1.7.0`. The local release candidate advances future corpus runs to patch release `tnm-research-pipeline/1.7.1` for structured signal/activity dates, explicit first activation and low/zero-yield readiness; the installed private research chain is aligned to its complete-run, exact-target, atomic-lineage, qualitative saturation and no-padding gates. Local skills fail closed until the deployed research contract advertises that patch and supports the candidate kind and schema; deployment does not grant staging, acceptance, publication, or template-activation authority.
