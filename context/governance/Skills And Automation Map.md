...
The deployed executable interoperability surface recognizes `organization_bundle_v3` and `organization_refresh_bundle_v2`; new production corpus campaigns require `tnm-research-pipeline/1.7.2`, including structured signal/activity dates, explicit first activation, low/zero-yield readiness, production-scale corpus segments, normalized source independence, reciprocal conflict lineage and role-specific planning. The tracked prepare command automatically verifies an equal or newer compatible production pipeline before creating a run, and import rechecks the actual candidate kinds and schemas immediately before intake. Operators do not synchronize patch strings manually; deployment does not grant staging, acceptance, publication, or template-activation authority.

The working-tree candidate advances the executable schema to
`tnm-research-pipeline/1.7.3` and adds the optional cited executive relevance
summary through Admin Review and both Publish paths. This is not the deployed
interoperability surface. No 1.7.3 run may be prepared or staged until the
ordered migration, compatible application and deployed contract are live
together; production and operator workflows remain on 1.7.2 until then.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| WF-01 | Workflow | Yes | `.agents/skills/tnm-autonomous-research/` | Andrew-invoked manual run | Production coverage, taxonomy, Source Book, durable sources | Deployed 1.7.2 comprehensive target lineage, exact dossier-readiness dispositions, and pending Admin Review candidates; 1.7.3 remains unreleased | May call the guarded staging RPC only after automatic deployed-compatibility checks; never accepts or publishes | Complete same-run specificity, coverage, qualitative saturation, production-corpus and staging gates; `pnpm research:validate` | Andrew reviews, accepts, and separately publishes; validated refresh edits require corrected artifacts and restaging | Active production on 1.7.2; exact live run/queue state must be read from Supabase | Andrew Davies | 2026-08-13 |
...
| WF-03 | Workflow | Yes | `.agents/skills/tnm-north-signal/` | Andrew-invoked weekly or manual preparation | Published Signals, original durable sources, published corpus changes, 28-feed register, selected discovery mail | Ignored private `north_signal_issue_v2` packet and feed-health reconciliation | Local private files only; no MailerLite campaign creation or sending | Registry health, six-section packet, Signals/source/link and no-Brief validation | Andrew edits, tests and sends in MailerLite | Active manual | Andrew Davies | 2026-08-10 |
...
| CT-01 | Executable contract | No | `app/src/lib/research/pipeline-schema.ts` | Research preparation, validation, smoke, and trusted import | Complete same-run typed artifacts | Canonical shape plus 1.7 record-specific cross-artifact validation; working-tree 1.7.3 adds the cited executive-summary outcome | Defines candidate shape and fail-closed intake eligibility | Research tests and `pnpm research:validate` | Deployed schema, Review and both Publish paths must advertise the same version before intake | Active production contract 1.7.2; 1.7.3 candidate unreleased | Andrew Davies | 2026-08-13 |
...
| EX-04 | External system | Yes | MailerLite True North Map workspace | Consent sync, webhook, automatic welcome, or Andrew campaign action | Supabase consent and Andrew-reviewed campaign | Subscriber group, lifecycle state, North Signal delivery | Delivery surface, not consent authority or auth mail | Live signup, sync, Gmail/non-Gmail delivery, image and link checks, webhook, unsubscribe and sender authentication | Andrew applies provider edits, tests and sends campaigns | Active production baseline; tracked restrained welcome and weekly source contracts were revised locally and still require explicit provider reconciliation/test before use | Andrew Davies | 2026-08-13 |
...
- North Signal changes: 28-feed registry and health reconciliation; v2 packet, durable-source, one-to-three Signals, Signals-link and no-Brief validation; acquisition-route consent/CTA/telemetry and QA-scorecard tests; tracked welcome/weekly presentation and link checks; Andrew still controls every MailerLite provider edit, test and send.
...
- Production releases: Node 24 and `pnpm release:validate` before one push to `main`; after the exact Vercel deployment is ready, run bounded `pnpm launch:validate`, affected-route/browser smoke, `/api/health`, deployment logs and live-state verification. Run the serialized `pnpm launch:audit` only for major sitemap/internal-link changes, scheduled assurance or an explicit broad-launch audit.
