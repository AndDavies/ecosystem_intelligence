...
The deployed executable interoperability surface recognizes `organization_bundle_v3` and `organization_refresh_bundle_v2`; new production corpus campaigns require `tnm-research-pipeline/1.7.3`, including structured signal/activity dates, explicit first activation, low/zero-yield readiness, production-scale corpus segments, normalized source independence, reciprocal conflict lineage, role-specific planning and an explicitly cited or null executive relevance summary. The tracked prepare command automatically verifies an equal or newer compatible production pipeline before creating a run, and import rechecks the actual candidate kinds and schemas immediately before intake. Operators do not synchronize patch strings manually; deployment does not grant staging, acceptance, publication, or template-activation authority.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| WF-01 | Workflow | Yes | `.agents/skills/tnm-autonomous-research/` | Andrew-invoked manual run | Production coverage, taxonomy, Source Book, durable sources | Deployed 1.7.3 comprehensive target lineage, exact dossier-readiness dispositions, cited executive decision snapshot outcome, and pending Admin Review candidates | May call the guarded staging RPC only after automatic deployed-compatibility checks; never accepts or publishes | Complete same-run specificity, coverage, qualitative saturation, production-corpus and staging gates; `pnpm research:validate` | Andrew reviews, accepts, and separately publishes; validated refresh edits require corrected artifacts and restaging | Active production on 1.7.3; exact live run/queue state must be read from Supabase | Andrew Davies | 2026-08-13 |
