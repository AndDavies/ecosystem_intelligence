...
`.agents/skills/tnm-daily-signals/` is the project-local workflow of record for the daily Canadian Defence Signals scan. Every Codex task opened in this project uses this canonical ignored skill rather than a cached or globally installed variant. The 07:00 Atlantic automation may publish only validated `daily_signals_packet_v1` editions to isolated `signal_*` tables through `app/scripts/publish-daily-signals.ts`; it never writes to organizations, capabilities, Public Needs, matches, citations, candidates, review queues, MailerLite, LinkedIn, or X. Every published edition requires one article-specific image from a cited durable source, verified for relevance and normalized to a 1600 x 900 WebP under the `signals/` prefix of the public `brief-images` bucket. Generic publisher backgrounds, logos, unrelated stock, image-search results, generated imagery, and image-less publication are prohibited. A no-publish day is valid and social copy remains private.
...
- `2026-08-04`: Reconciled the local Daily Signals skill and scheduled automation after credential setup. The skill now documents the root `pnpm signals:publish` path, ignored `app/.env.local` loading, fail-closed service-role handling, and `America/Halifax` cadence; automation `true-north-map-daily-signals` remains active daily at 07:00 with the same explicit credential, source, image, idempotency, and publication-boundary gates. A dry-run and idempotent apply from the scheduled project's exact working directory both passed against the already-published edition.
