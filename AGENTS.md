...
`.agents/skills/tnm-daily-signals/` is the local workflow of record for the weekday Canadian Defence Signals scan. It is separate from research ingestion, visibility, and weekly North Signal. It may publish only validated `daily_signals_packet_v1` editions to isolated `signal_*` tables through `app/scripts/publish-daily-signals.ts`; it never writes to organizations, capabilities, Public Needs, matches, citations, candidates, review queues, MailerLite, LinkedIn, or X. A no-publish day is valid and social copy remains private.
...
- `2026-08-03`: Released the isolated weekday Canadian Defence Signals system. Descriptive immutable `/signals/[slug]` editions separate public-source facts, automated reads, unknowns, next steps, source links, and existing atlas relationships. The production migration and foreign-key indexes are reconciled to the live ledger; RLS exposes published content while keeping run health and social drafts private. The executable editorial gate requires an executive field-guide narrative, deterministic story order, five-to-eight distinct developments, original durable sources, and idempotent publication. The first approved edition is published; no schedule, MailerLite campaign, LinkedIn post, X post, or core-corpus write was activated.
