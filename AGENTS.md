...
- `2026-08-02`: Rebalanced the approved guided landing and atlas workspace around the mission-to-engagement outcome. The landing now presents a static, source-backed Kraken Robotics and KATFISH specimen before the worked example without initializing MapLibre, while `/map` places the live national map in the first viewport with a compact control band, an internally scrolling 380-pixel results rail, the existing accessible evidence table, and explicit mobile Map/List plus collapsed, preview, and expanded result-sheet states. Bounds, query, selection, guided-example, sharing, refresh, browser-history, authentication-return, Ask quota, Working List, and publication contracts remain unchanged; no database, provider, research-skill, scheduled-task, or public API change was introduced.
