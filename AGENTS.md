...
`.agents/skills/tnm-daily-signals/` is the project-local workflow of record for the daily Canadian Defence Signals scan. Every Codex task opened in this project uses this canonical ignored skill rather than a cached or globally installed variant. The 07:00 Atlantic automation may publish only validated `daily_signals_packet_v1` editions to isolated `signal_*` tables through `app/scripts/publish-daily-signals.ts`; it never writes to organizations, capabilities, Public Needs, matches, citations, candidates, review queues, MailerLite, LinkedIn, or X. A no-publish day is valid and social copy remains private.
