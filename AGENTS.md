...
Current release state: production soft beta. The deployed public application and canonical production database define the current product. Broader promotion remains gated by live queue triage, field-performance review, and the active security and reliability register; a standing screenshot or launch packet is not a product dependency.
...
- Treat production as the current product reference, then reconcile tracked code and governance to it. Never use an old screenshot, deck, report, or launch packet to restore superseded routing, copy, or design.
- Create screenshots, mockups, decks, reports, and launch collateral only when Andrew explicitly requests that deliverable. Keep them local by default; do not add generated collateral to the repository merely as task evidence.
...
## Daily Signals Editorial Workflow

`.agents/skills/tnm-daily-signals/` is the project-local workflow of record for the daily Canadian Defence Signals scan. Every Codex task opened in this project uses this canonical ignored skill rather than a cached or globally installed variant. Scheduled and Andrew-invoked edition-producing runs may publish only validated `daily_signals_packet_v1` editions with six to eight distinct signals to isolated `signal_*` tables through `app/scripts/publish-daily-signals.ts`; they never write to organizations, capabilities, Public Needs, matches, citations, candidates, review queues, MailerLite, LinkedIn, or X. Every published edition requires one article-specific image from a cited durable source plus at least one current-edition LinkedIn example and one X example stored privately in `signal_social_drafts`. `/admin/signals` lists editions and run health; `/admin/signals/[id]/edit` provides the owner-only page editor, source and hero provenance, atlas continuations, and view-and-copy-only social examples. Generic publisher backgrounds, logos, unrelated stock, image-search results, generated imagery, image-less publication, and publication without both social examples are prohibited. The image is normalized to a 1600 x 900 WebP under the `signals/` prefix of the public `brief-images` bucket. A no-publish day is valid and social copy remains private.
...
- Search entity-outward from organizations to specific products, variants, subsystems, interfaces, partners, programs and proof events, and problem-inward from Mission Areas and published Public Needs through outcomes, constraints, metrics, standards, procurement language, and bilingual terminology to candidate capabilities and enabling organizations.
- Every selected candidate must expose a compact decision chain in existing review fields: specific capability or need, coverage value, evidence composition, current trigger when one exists, conservative Mission/Public Need read, material unknowns, and one bounded reviewer action. Public Need hypotheses remain Derived Reads and do not create capability-demand matches outside the existing private matching workflow.
- Treat a dossier as saturated only when two additional complementary lanes do not materially change the capability definition, proof/current state, Mission/Public Need read, unknowns, or reviewer action. Repeated sources and added prose do not establish saturation.
...
- `2026-08-06`: Tightened the local autonomous-research coordinator and tracked executable contract without changing its review-first authority. Pipeline `1.5.0` requires atomic leaf-field claim targets, real lifecycle timestamps, one concise five-part reviewer rationale, record-specific warnings, and refresh signal artifacts; shared trust boundaries stay at the packet level. Repository validation is summary-first unless `--verbose true` is requested. Twelve completed August 5-6 run families were reconciled to production and retained as immutable lineage after all 86 associated candidates were confirmed published and the pending Review queue was confirmed empty. The canonical local skills remain ignored and no research run, database write, review decision, or publication occurred during this hardening pass.
- `2026-08-05`: Repaired the autonomous-research smoke preflight so its deployed Review and Publish compatibility check always uses the canonical production contract instead of inheriting the browser-facing `NEXT_PUBLIC_SITE_URL` used by local development. A stopped localhost server can no longer create a false intake failure; incompatible or unavailable production contracts still fail closed, and research, review, publication, schema, and database authority remain unchanged.
- `2026-08-05`: Strengthened the manual review-first research chain for decision usefulness without changing its schemas, database authority, or publication boundary. New runs now search both entity-outward and problem-inward, decompose independently reviewable capabilities and variants, pair capability evidence with official Mission Area or Public Need premises, classify consequential unknowns, measure saturation by marginal decision value, and require every candidate rationale to expose coverage value, evidence, a conservative Mission/Public Need read, unknowns, and one bounded reviewer action. Exact Public Need hypotheses remain private Derived Reads until a reviewed published capability enters the existing demand-matching workflow.
- `2026-08-05`: Reconciled production, tracked application state, local operator systems, and active governance. Retained two completed research runs as validated immutable lineage after confirming their candidates had already passed human review and publication; kept reusable visibility tooling while raw provider material remained ignored; retired the autonomous broad-research schedule; added a conventional owner Signals index and edition editor with view-and-copy-only social examples; removed obsolete tracked launch packets, lookbooks, audits, and beta-plan exports; and added a repository hygiene gate plus explicit-request-only collateral rule. No schema, canonical-record, authentication, provider, consent, or publication-authority change was introduced.
- `2026-08-05`: Made current-edition LinkedIn and X examples deterministic Daily Signals publication outputs rather than optional follow-up. The packet contract now requires both platforms, the publisher verifies them before a new edition becomes public and repairs missing rows during an idempotent rerun, run reports expose the verified count and platforms, and `/admin/signals` labels examples by edition and status without caching the owner view. Scheduled and Andrew-invoked edition-producing runs use the same skill and publisher; neither gains authority to post externally or write outside isolated `signal_*` tables.
- `2026-08-05`: Activated the owner-authorized Search Console BigQuery bulk export for the verified URL-prefix property in the Montréal dataset, granted the export service and private visibility collector the required least-privilege BigQuery roles, and aligned the Monday visibility automation with configured-provider strictness. Optional-unconfigured APIs such as Bing remain unavailable/unknown without failing refreshes; configured provider failures still block, while valid CrUX no-data and Google's documented first-48-hour BigQuery table warm-up remain explicit non-blocking states. The visibility boundary remains private/local and no Supabase, public content, indexing, outreach, or publication authority changed.
- `2026-08-05`: Completed global-refinement Phase 6 across shared system states, supporting-route metadata, breadcrumbs, and active governance. Loading shells remain fixed-geometry and literal; empty states are scoped and tonal; errors offer retry and Map recovery; the noindex 404 offers Map and homepage recovery; and supporting trust routes use Home breadcrumbs with current social metadata. Canonicals, sitemap, authentication, analytics, Supabase, research, review, publication, and provider boundaries are unchanged.
- `2026-08-05`: Aligned Daily Signals to the updated 06:30 America/Halifax schedule and raised the executable publication floor to six signals while retaining the eight-item editorial quality ceiling. The scheduled job now prefers seven or eight when independently supported. Formalized Morning Brief `related_links` as a compact list of canonical, source-resolved articles that clear the relevance gate but do not earn a full anchor or Radar entry; the existing PDF, Markdown, archive, and validator paths now describe that list explicitly.
- `2026-08-05`: Completed global-refinement Phase 5 across the guided landing, How It Works, About, Methodology, contribution, sign-in, account, contact, and Working List journeys. About now leads with the canonical founder story and wording; How It Works moves plainly from a question through published capability and evidence into a Working List or conversation; Methodology remains the detailed review reference and ends with a published-profile inspection path; and contribution copy uses the approved public-record promise. The landing architecture, cached public shell, client-hydrated authentication, North Signal consent and delivery, private-route access, analytics, Supabase, research, review, and publication contracts are unchanged.
- `2026-08-05`: Completed global-refinement Phase 4 across the Signals and Defence Briefs editorial surfaces. Signals now uses a concise decision-useful archive summary and a direct continuation into Defence Briefs, while Defence Briefs adopts the same source-linked editorial hierarchy, tonal surfaces, rounded geometry, and existing record and Mission Area continuations. The work changes presentation and public copy only: article bodies, cited imagery, generated anchors, publication state, Daily Signals automation, editorial skills, MailerLite, Supabase, analytics, research, review, and publication authority remain unchanged.
- `2026-08-04`: Reconciled the local Daily Signals skill and scheduled automation after credential setup. The skill now documents the root `pnpm signals:publish` path, ignored `app/.env.local` loading, fail-closed service-role handling, and `America/Halifax` cadence; automation `true-north-map-daily-signals` remains active daily at 06:30 with the same explicit credential, source, image, idempotency, and publication-boundary gates. A dry-run and idempotent apply from the scheduled project's exact working directory both passed against the already-published edition.
- `2026-08-03`: Rebuilt the reusable Canadian Defence Signals edition as a wide executive briefing and aligned the archive with the shared tonal interaction system. Editorial Blue now organizes summaries, sources and navigation; feature and article cards use consistent rounded borderless surfaces; taxonomy and link pills retain a quiet neutral edge; hover states never move cards or arrows; and direct LinkedIn and X actions replace the extra share step. The change is presentation-only: autonomous content generation, source data, publication authority, newsletter delivery and the core atlas remain unchanged.
- `2026-08-03`: Made cited editorial imagery a deterministic Canadian Defence Signals publication gate. The local skill now extracts and compares publisher-declared candidates, rejects generic share backgrounds and undersized assets, requires visual relevance review, and blocks image-less editions. The publisher always normalizes the selected source image to a 1600 x 900 WebP in `brief-images` and supports a narrow idempotent `--replace-hero` repair for existing editions. All six published editions were reconciled to sourced stored images without changing their slugs, article text, sources, links, publication status, or core atlas data; the active Atlantic automation now enforces and verifies the same image contract.
- `2026-08-03`: Released the isolated Canadian Defence Signals system and activated its project-local daily Atlantic automation after production schema, RLS, routes, publication, sitemap, and health verification. Descriptive immutable `/signals/[slug]` editions separate public-source facts, automated reads, unknowns, next steps, source links, and existing atlas relationships. The executable editorial gate requires an executive field-guide narrative, deterministic story order, six-to-eight distinct developments, original durable sources, and idempotent publication. A no-publish day remains valid; MailerLite, LinkedIn, X, and the core atlas remain outside the automation's authority.

- `2026-08-02`: Completed a release-hardening reconciliation without changing public data, review, publication, authentication, or research authority. National discovery now assembles the complete corpus from independently cached 1,000-row relation pages under the shared `atlas-public` tag; public retry uses bounded jitter; middleware is limited to routes that need compatibility or authentication handling; and shared navigation uses Inter while Barlow remains a deliberate brand display face. Node 24, release CI, CodeQL, full dependency auditing, Dependabot vulnerability alerts, secret scanning, and push protection establish the tracked software-assurance baseline without creating automated update branches. Migration filenames now match the live Supabase ledger without executing SQL. Superseded release exports and private generated material were removed from the active tracked tree, historical governance moved to a non-operating archive, and a log-only Vercel Firewall observation rule remains staged for owner publication. Codex Security authentication and scan remain the next assurance step.
- `2026-08-02`: Rebalanced the approved guided landing and atlas workspace around the mission-to-engagement outcome. The landing presents a source-backed Kraken Robotics and KATFISH specimen before the worked example, using a lazy fixed MapTiler view with Kraken selected and every map interaction disabled; `/map` places the fully interactive national map in the first viewport with a compact control band, an internally scrolling 380-pixel results rail, the existing accessible evidence table, and explicit mobile Map/List plus collapsed, preview, and expanded result-sheet states. Bounds, query, selection, guided-example, sharing, refresh, browser-history, authentication-return, Ask quota, Working List, and publication contracts remain unchanged; no database, provider, research-skill, scheduled-task, or public API change was introduced.
- `2026-08-02`: Standardized shared public navigation on the approved interface typography and removed the superseded COVE-era brand audit. Repaired the refresh Admin Review intake dependency with a least-privilege grant that allows only `service_role` to execute the private immutable baseline parser; anonymous and ordinary authenticated roles remain denied. The validated North Vector Dynamics refresh later passed human review and publication through the unchanged checkpoint.
...
- `2026-07-26`: Completed Phase 2 broader-release hardening. The public atlas now sends a compact national marker/search projection while rich cards remain paginated, retries bounded transient public reads, retains a warm in-process safe snapshot for temporary upstream failures, treats invalid refresh tokens as signed-out sessions, exposes a private-detail-free health endpoint, applies a provider-specific CSP, schedules privacy-retention cleanup, preserves bounded campaign attribution, and adds a repeatable low-rate public launch crawl. The dated launch collateral created during that phase was later retired from the active tracked tree.
